...
const seedChangelog: ChangelogEntry[] = [
  ...
  },
  {
    ...
    date: "21 June 2026",
    version: "1.3.0",
    title: "Admin module - top-tab rebuild + email/password auth + per-user portal + bonus log",
    changes: [
      { type: "added", text: "Admin top nav consolidated to 3 tabs: Inbox / People / Hiring (+ Settings link). Switching tabs is local state, not route navigation - instant swap, no scroll reset, no remount. Detail routes (/company/people/[id], /company/contracts, /company/structure, /company/bonuses) still own the body when active. Sidebar entry stays as Admin → /company" },
      { type: "added", text: "Inbox panel replaces Overview - the action queue. Five sections: Contracts awaiting your counter-sign, Scoring drafts ready to lock, 30-day reviews coming up, Bonuses due (with auto-computed amount + Approve button), KPI bonus suggestions (auto-detected from kanban test wins, +5% of monthly comp). Plus a Recent activity feed of the last 10 contract / bonus events, and a cross-cutting links row to All contracts / bonuses / invoices" },
      { type: "added", text: "People panel gets 4 view modes: Grid / Table / By pod (replaces the Structure flowchart) / By status. Each pod card lists its 4 slots with status pills; By status groups onboarding / active / on leave / notice / offboarding / left. Add Person modal gains a one-step Send invite to launchpad checkbox - creates the Person and emails the invite in one shot" },
      { type: "added", text: "Per-person profile now carries every HR facet inline. New tabs: Onboarding (30-day clock + 7-step default checklist with overdue flagging), Invoices (filtered company_invoices via linked_person_id with billed/paid/outstanding header), Bonuses (audit log + log-a-payment form, Scheduled vs Paid badges), Scoring (contractor scheme auto-compute), KPIs (read-only delivery summary). Linked pod member picker on Overview lists every pod member by pod + role and flags any already-linked to a different Person" },
      { type: "added", text: "Engagement type on Financial: core_retainer (revenue-tier bonus inputs at 100k / 150k / 200k) / contractor_retainer (max +33% / -30% per month) / contractor_per_page (max +25% / -30% per build). The Scoring tab only shows for the two contractor types; the revenue-tier inputs only show for core_retainer" },
      { type: "added", text: "Contractor scoring engine per the contractor scheme doc. Per kanban deliverable (per-page) or per month (retainer) the engine auto-derives entries from kanban_tasks: delivered-early bonus, zero-revisions bonus, test-win bonus, late deductions (1-2d -5%, 3+d -15%), 3+ revision rounds deduction. PM can add manual entries (renewal, no-show, complaint). Caps applied at lock time. Lock attaches to an invoice. Bonuses default to pay-date = 28th of next month so admin approves now and the cash leaves on the regular cycle, not with 2 days notice" },
      { type: "added", text: "Email + password auth as the primary sign-in (magic link still available as a fallback, admin shared password still works for Dylan). Forgot password link emails a reset to /login/reset-password. Server-side /api/admin/invite-user route uses the service-role key to inviteUserByEmail + add to app_users allowlist - the only way into auth from our UI is admin-initiated. Access stays locked to the allowlist on every sign-in path" },
      { type: "added", text: "Per-user /me portal for team members. Lands here after sign-in. Contract sign nudge at the top when applicable, four cards: My tasks (links to /my-work), Submit an invoice (/me/invoices - upload form that auto-sets linked_person_id), My profile (/me/profile - read-only view of their own Person record), Change password (emails a reset link). Team members can't reach /company/* but get a slimmed-down view of their own slice" },
      { type: "added", text: "Global Bonuses log at /company/bonuses - cross-person view of every BonusPayment with per-currency totals, kind + status filters, click-through to the person's Bonuses tab. Reachable from Inbox" },
      { type: "added", text: "Hiring kanban columns coloured (indigo / blue / amber / orange / green from Applied → Hired) and restyled to match the Project Delivery kanban shell. Hired auto-fires the launchpad invite when the candidate has an email" },
      { type: "added", text: "Pod roster editor in /pods-v2/admin swaps the inline rename input for a Person picker. Picking a Person writes person_id (the bridge to admin/People) + mirrors the current full_name into PodMember.name for backward compat. Unlinked + placeholder members get badge chips so the team can see what needs filling. PodMember.person_id added to the type" },
      { type: "added", text: "Person rename in Admin propagates to kanban_pods + kanban_tasks (via Supabase update) + pods-v2 PodMember.name (via localStorage write + sync) + settings.team (legacy directory). Best-effort fan-out so admin doesn't have to chase the name across surfaces" },
      { type: "added", text: "Person.status = left now cascades to app_users.active = false (the launchpad allowlist) so leavers can't sign in. Flipping back to active re-enables. KPI bonus suggestions cross-check scoring_periods to suppress double-pay if the same test win was already included in a scoring period" },
      { type: "improved", text: "Agreements collapsed from NDA + Contract to one master Contract template. Confidentiality clauses absorbed inline as §7a / §7b. Generate / quick-add modals no longer offer NDA. Legacy NDA rows in the DB still display via the read path; no data loss. Master template stays the editable doc with placeholders for name / role / comp / start_date / signature so each contract is just per-person fills" },
      { type: "improved", text: "Person profile drops Emergency contact (out of scope), drops Performance review fields cleanup follow-up. STATUS_BADGE gains onboarding (indigo) and offboarding (orange). Onboarding checklist task copy updated to reference the master Contract, not the old NDA + Contract pair" },
      { type: "improved", text: "Company passcode moves from hardcoded Football2026 to NEXT_PUBLIC_COMPANY_PASSWORD env var with the literal as fallback" },
      { type: "fixed", text: "Invite button on Person profile detects an existing app_users row and shows Already invited instead of letting admin double-fire. Add Person checkbox skips the invite call when no email is set" },
      { type: "fixed", text: "Standalone /company/invoices removed from top nav (the per-Person Invoices tab is the useful view). Reachable from Inbox + each Person's Invoices tab. Finance module untouched" },
    ],
  },
  {
    id: "cl-89",
    date: "21 June 2026",
    version: "1.2.0",
    title: "HR / Admin module - per-person hub fleshed out end-to-end",
    changes: [
      { type: "added", text: "Person profile now carries every HR facet for one human in one place. New tabs: Onboarding (30-day clock + seeded 8-step checklist with overdue flagging), Invoices (filtered company_invoices via linked_person_id with billed / paid / outstanding header), Scoring (contractor scheme auto-compute), KPIs (read-only delivery summary). Agreements tab unchanged - the existing NDA + Contract lifecycle (draft, sent, team-signed via /portal, counter-signed via /company/contracts, active) was already complete" },
      { type: "added", text: "Engagement type field on Person.Financial: core_retainer (revenue-tier bonus inputs at 100k / 150k / 200k), contractor_retainer (max +33% / -30% per month under the contractor scheme), or contractor_per_page (max +25% / -30% per build). The Scoring tab only shows for the two contractor types; the revenue-tier inputs only show for core_retainer. No abstract toggles, one engagement choice drives every other surface" },
      { type: "added", text: "Contractor scoring engine. Per kanban deliverable (per-page) or per month (retainer) the engine auto-derives the contractor scheme entries from kanban_tasks: delivered-early bonus, zero-revisions bonus, test-win bonus, late deductions (1-2d -5%, 3+d -15%), 3+ revision rounds deduction. PM can add manual entries (renewal, no-show, complaint) the data can't see. Caps applied at lock time (+25/-30 per-page, +33/-30 retainer, +15 monthly test-win sub-cap). Lock attaches to an invoice; 5-day dispute window documented in the footer" },
      { type: "added", text: "Per-person KPIs tab: delivered count, on-time rate, currently overdue, 3+ rev rounds, tests run/won/win rate, late deliveries. Plus a scoring rollup showing locked-period count + total delta % applied + average per period. All numbers derive from the same kanban data the Scoring tab uses, no source-of-truth drift" },
      { type: "added", text: "Person.status gains onboarding + offboarding. STATUS_BADGE updated to include both. Status select on Overview now lists all six. Onboarding tab only surfaces when status === 'onboarding' or onboarding_started_at is set, so active staff don't see it" },
      { type: "improved", text: "Standalone Invoices tab removed from /company top nav (the flat list duplicated /finance/invoices conceptually). Reachable from the per-Person Invoices tab via 'Manage all invoices →'. Finance untouched" },
      { type: "improved", text: "Company passcode reads from NEXT_PUBLIC_COMPANY_PASSWORD env var with a fallback to the prior literal so existing sessions keep working" },
    ],
  },
  {
    id: "cl-88",
  ...
  },
  {
    ...
    date: "21 June 2026",
    version: "1.1.0",
    title: "Mission Control kanban rebuild + sidebar / shortcuts / dark-mode polish",
    changes: [
      { type: "added", text: "Mission Control kanban (/kanban) rebuilt from scratch. Three view modes (By project / All projects / By pod / Results Library), all sharing the same 8 phase columns plus a Tickets and Documents lane. Cards carry phase, category, designer + developer pairs, brief URL, Figma URL, due date, notes, and a per-card metrics array for tests. Drag-and-drop between columns, card detail modal with everything inline, custom dark date picker (portal-rendered so it never gets clipped inside the modal). Runs on mock data today; Supabase wiring lands in a follow-up" },
      { type: "added", text: "Project types - Build (15 / 20 / 25 day turnaround) or Retainer (30 / 60 / 90 day engagement). Retainers auto-seed a Documents schedule (weekly reports + monthly test plans + monthly results writeups, all dated forward) so the team isnt staring at an empty board on day one. Build pills are white, retainer pills are teal so the eye reads them at a glance" },
      { type: "added", text: "Dated deadline engine for Build projects. Each phase has a clean integer day budget (Strategy 2d, Design 4d, etc), totalling internal target = turnaround - 3 days. Phase 1 dates lock at project start, Phase 2 dates lock when the PM marks the client as approved (via the date picker in the Build schedule, can backdate). External Revisions runs on a 48h client clock anchored to sent-to-client. Card status colours (on-track / amber / red) drive off the current phase due date vs today. Late-Phase-2 warning banner fires when Launch and Testing lands past the client deadline" },
      { type: "added", text: "Internal Revisions sign-off flow. In the card modal, Approve turns the card green, keeps it in Internal Rev, flips the assignee to the primary designer (so they know to send it to the client), with an Undo. Request revisions bounces back to Design with a Revisions Needed pill auto-applied. QA mirrors the same pattern - Approve forwards to Launch and Testing, Send back bounces to Dev with the tag. Both flows fire the notification stub (Slack wiring comes later via Viktor)" },
      { type: "added", text: "Launch and Testing flow. Test setup in the modal: live URL, tracked metric, baseline, multi-metric tracking, interim notes, screenshot upload, Set Live + Conclude buttons. Live cards pulse green on the board with a Live badge; auto-prompts to log interim after 5 days and to conclude after 14. Concluded tests fall off the active board into the Results Library view, filtered by outcome / client / metric" },
      { type: "added", text: "Tickets column with per-category SLAs - Ticket gets 48h to amber, 72h to red; Bug 24h / 48h; Fire is stuck the moment it lands. Each ticket gets a Complete button (no phase progression, just close in place) with a Reopen if marked done by mistake. Bug / Ticket / Fire icons render in the card header for at-a-glance triage" },
      { type: "added", text: "Shortcuts module (/shortcuts) - new sidebar item between My Tasks and Project Delivery. Acts as an umbrella with a tab strip across every underlying tool route (Payment Links, Price List, Invoice Generator, Case Studies, About Deck, Portfolio) so navigating between them feels in-page. Pitch nav slimmed down to Offer / Quotes / Sales Deck / Cheat Sheet to remove the overlap" },
      { type: "added", text: "Custom dark confirm modal replaces window.confirm for destructive actions (delete card, overwrite pod assignees). Promise-based confirmAction helper. Pod reassignment now prompts before overwriting per-card assignees - cancel to set the pod without touching cards" },
      { type: "improved", text: "Notifications fire on every assignee change - phase moves, new card creation, approve / request revisions, plus a single batched ping per pod reassignment instead of one per card. All currently stubbed; Slack hook drops in cleanly when Viktor wires it" },
      { type: "improved", text: "Sidebar restructure: My Tasks at the top, Shortcuts cluster, then the lifecycle group (Project Delivery, Onboarding, Acquisition, Retention), Training, Team utilities, Finance + Admin. Workspace re-pinned alongside Project Delivery until pods-v2 migration is wired" },
      { type: "improved", text: "Pitch + Shortcuts umbrellas now persist their tab strips across underlying routes via PitchShell + ShortcutsShell in the dashboard layout. No remounting between tabs - feels like one cohesive surface" },
      { type: "improved", text: "Onboarding brief popup on the kanban now renders the structured Q&A from the OnboardingSubmission (target customer, voice, USPs, success metric, etc.) instead of iframing a URL. Five grouped sections, scrollable, read-only" },
      { type: "improved", text: "Project tab pills carry a turnaround badge (15d / 20d / 25d for builds, 30d retainer etc) plus an open-count, so the eye reads scope and load at the same time" },
      { type: "fixed", text: "Login screen submit buttons (Email me a sign-in link / Continue) were dark text on a near-black background after the dark-mode sweep - invisible. Flipped to white pill with dark text matching the active-pill pattern elsewhere" },
      { type: "fixed", text: "Right-side cluster in the kanban header (count / phase rules / pill toggle / dropdown) reflowed when switching view modes because content widths varied. Now uses a 2-col grid with a fixed-width slot for the selector dropdown - toggle position is anchored across every mode" },
      { type: "fixed", text: "Project Tabs Row, Pod Projects Row, and Clients Row used flex-wrap, so long lists would wrap to a second line and push the board down. Switched to overflow-x-auto so they scroll horizontally instead" },
      { type: "fixed", text: "Strategy Brief and Figma URL chips in the card modal had identical edit chrome but lived in two awkward sections - unified to a single Strategy brief slot plus a dedicated Figma slot with pencil-to-edit" },
      { type: "fixed", text: "Many internal pages still carried light-mode tokens after the previous dark-mode sweep (bg-white on tooltips, light dividers, etc). Cleaned 50+ remaining instances across dashboard, team, sales-engine, and components. Skipped (public) and (portal) - those stay light for clients" },
      { type: "removed", text: "Native browser date picker in the card modal - the styled overlay hack was buggy. Replaced with a custom DarkDatePicker (month grid, Mo-Su, Today + Clear shortcuts) portal-rendered so the popover never gets clipped inside scroll containers" },
    ],
  },
  {
    id: "cl-84",
  ...
  },
  {
    ...
    date: "1 June 2026",
    version: "1.0.0",
    title: "Launchpad v1.0: per-person login, the Workspace, and one tool for the whole team",
    changes: [
      { type: "added", text: "Per-person login (magic link). Everyone signs in with their own email instead of a shared access code, so every action is attributable to a real person. Backed by an app_users allowlist (migration 024): only invited emails can sign in, no open signup. Each account links to its existing pod-member record, so a person's assigned tasks light up the moment they log in with zero data migration. The old shared passwords still work as a transition fallback so nobody gets locked out during cutover. Admin manages access at Workspace > Team access: invite by name + email + role, link to a pod member, revoke or restore. Founder seeded as the first admin" },
      { type: "added", text: "The Workspace (/workspace): a clean, from-scratch pods + clients + delivery area that becomes the operating surface for the agency. Overview shows the whole agency at a glance — at-risk / due-this-week / awaiting-client / tests-needing-a-call stat tiles, a compact click-to-expand week strip, a decluttered 'needs attention' feed grouped one-row-per-client, a pods snapshot, and a client-health strip. Built on the existing pods-v2 + strategy data, so it renders live client data with no new plumbing" },
      { type: "added", text: "Workspace Pods: a single-column pod list with capacity shown three ways (this week / next week / this month) plus retainer vs sprint split. Each pod detail page has a Mon-Fri weekly calendar of what's due (with owner per item for nudges), capacity, a Retainers section and a Sprint work section, and the team roster on the right" },
      { type: "added", text: "Workspace Clients: retainers and one-off sprints split into clear sections, each card decluttered to read at a glance — health dot, one status signal, and a time signal that fits the engagement type (prominent Day X/90 for retainers, start date for sprints). Retainers get a subtle darker surface + a RETAINER chip so the two groups are instantly distinguishable" },
      { type: "added", text: "Client detail is now the home for an engagement (replacing the old /engagements page). Objective front-and-centre and editable; the full 14-field client brief surfaced as a collapsible, editable panel; a Strategy section that is where deliverables are ADDED (pick page + weight, it spawns the paired design + build tasks); strategist docs + per-page briefs; an editable Tests tracker (name, metric, scheduled/live/complete, winner/loser/inconclusive, lift %, link to the test); and an editable Delivery kanban for design + dev with mark-done" },
      { type: "added", text: "Strategy-driven deliverables with a two-role flow: a deliverable is entered (saves immediately, brief decision pending), then the strategist rules on each one with Attach brief / No brief needed. Adding a deliverable spawns the paired design + build tasks" },
      { type: "added", text: "Approval-gated dev clock: a build task doesn't start its countdown until the client approves the paired design. While unapproved it reads 'Awaiting approval' and is excluded from overdue / at-risk / the week strip. A manual 'Client approved' button on the design stamps the date and starts the dev clock — dev due = approval + the project's bucket turnaround (A 4 days / B 6 / C 8 working days), with the first one or two heavy deliverables fast-tracked to ~4 days so clients see early value on big builds. The project's bucket recomputes as deliverables are added so turnarounds stay accurate" },
      { type: "added", text: "New clients flow straight from the onboarding inbox into the Workspace: approving a submission creates the client (brief pre-filled) and lands on its Workspace page. A parked client (no pod yet) shows an Assign to pod step — pick a pod + retainer/sprint and it creates the engagement so the strategy deliverable-adder is immediately usable" },
      { type: "added", text: "My Work (/my-work): the personal landing for everyone. Resolves the signed-in person and shows the deliverables assigned to them across every client, grouped Overdue / Due soon / Upcoming / Waiting / Done, with mark-done. Members land here on login" },
      { type: "improved", text: "Team Hub retired — one tool for everyone. The separate /team hub and its sidebar are gone; members now use the same dashboard shell as admins, with the nav split by role. Members see Mission Control, My Work, Workspace, Wiki, their pod tiles, and a collapsible Toolbox (Swipe File, Font Library, Submit Invoice, Payments). Admins additionally get Offer, R&D, and the Acquisition / Delivery / Operations / Finance / Company sections plus Settings. Members are scoped to their own pod in Workspace and can't reach admin pages (they're redirected to My Work)" },
      { type: "improved", text: "Navigation tidy: removed the standalone Pods and Clients pins (Workspace covers them), shelved Agents, softened the section dividers to subtle hairlines, and collapsed the member tools into one logically-ordered Toolbox group" },
      { type: "improved", text: "Mission Control restyled to match the Workspace look — calm slate cards, consistent headings and section labels — so the whole app reads as one product" },
      { type: "improved", text: "Offer page: dark above-the-fold Conversion Engine hero with a clear Regular vs Pro distinction, the buy-in highlighted as the recommended pick" },
      { type: "added", text: "Quotes tool (Acquisition > Quotes): build a client-facing proposal from free-form line items (one-off or monthly), each with an optional one-line detail, and generate a shareable link clients open as a clean HTML page — not a PDF. The proposal leads with 'A conversion plan for [client]', lists the plan with split one-off / monthly totals, and shows a tight 'Every engagement includes' checklist that comes pre-filled (Strategic CRO, custom code development, conversion-focused design, 30 days of post-launch support) and is editable per quote. You only really type the client name, an intro line, and the deliverables. View-tracking stamps when a client first opens it; quotes can be trashed and restored (migration 025)" },
      { type: "added", text: "Pause a client: a Pause control on the client header freezes the engagement, shows a Paused pill + banner, and on resume shifts every open deadline forward by the number of days paused so nothing reads as falsely overdue after a client-side hold" },
      { type: "added", text: "Multiple briefs per deliverable: the brief panel now supports linking several reference briefs (label + optional URL) alongside the primary brief, add/remove inline" },
      { type: "added", text: "Amends lane on client delivery: secondary tickets and amends live in their own section, separate from the core strategy-driven deliverables, so post-launch tweaks don't clutter the main build flow" },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "improved", text: "Sidebar cleanup: trimmed each lifecycle lane to what we actually use day to day. Acquisition now shows just Portfolio and Case Studies (Audits, Social, and Proposals moved to Shelved). Delivery shows just Onboarding (Portals and Task Board shelved). Operations shows just Feedback (Tickets shelved). Wiki was promoted out of the Operations dropdown to a pinned top-level link sitting between Pods and R&D, since the team hits it constantly and the extra click was friction. The big Shelved drawer is gone; in its place there's a small 'Shelved' link next to the version footer that opens a read-only list of parked tools. All shelved routes still exist in git, they just aren't surfaced in the nav, so we can promote any of them back without rebuilding" },
      { type: "added", text: "Command palette (⌘K from anywhere): fuzzy-searches every nav route, with arrow-key navigation, Enter to jump, and Esc to close. Shelved tools are deliberately excluded from the index so parked routes stay unreachable. Trigger sits at the top of the sidebar as a 'Search ⌘K' pill" },
      { type: "added", text: "Pod identity tiles: the three pods now show as colored mini-tiles (peach Pod 1, blue Pod 2, mint Pod 3) between the pinned items and the lifecycle lanes, jumping straight to each pod page. Picked up the pattern from ContrastAI's Spaces section, mapped to our own data" },
      { type: "improved", text: "Removed the Tickets list from Mission Control since the same view lives on the Task Board and the duplication wasn't earning its space" },
      { type: "removed", text: "App Switcher (Launchpad / Sales Engine dropdown) pulled from the sidebar header now that Sales Engine isn't an active surface. Component file kept in git for the day we want a multi-app shell again" },
      { type: "improved", text: "Sidebar visual overhaul: shifted to a dark charcoal shell (#0E0F11) with white workspace label and a small green 'live' indicator at the top. Active items use a high-contrast white pill, inactive items are zinc-200, hover lifts to white over a subtle bg-white/[0.06] surface. Solid heroicons replace the outline set so they hold up against the dark bg. PRIVATE/ADMIN/PARKED badges replaced with a small lock-icon chip in a neutral surface, killing the warm orange that clashed. Team Hub got a distinct gradient-and-border treatment with a hover arrow so it reads as a primary call-to-action rather than another nav row. Section header chevrons brightened to white/30 (white/60 on hover) so they're actually visible. Scrollbar hidden via [scrollbar-width:none] so the sidebar stays chrome-free. Single left rail across the whole sidebar (pinned items, Pods label, Pod tiles, lifecycle/ops section headers, Agents, Settings) so every label hangs off the same vertical line. Dashboard layout container matched to the same dark so the main white content area reads as a floating card with a rounded left corner" },
      { type: "added", text: "Wiki v2 (V1 content release) at /wiki-v2. Brand-new wiki built from scratch as the directional + operational brain of the agency. 27 pages organised across 9 sections: Start Here, Culture (Values, How we work), People & Pods (The team, Pod model, Hiring), Communication (Slack, Standups, Client comms), Sales (Pitch overview, Cheat sheet, Objection bank, Offer, Positioning, Audit playbook, Proposals), Delivery (Conversion Engine methodology, Design process, Figma workflow, Handoff to dev, Build process, Shopify workflow, Deployment, Performance, Audit process, Conversion Matrix, Testing framework, Reporting, QA checklist), Client Engagement (Client onboarding, Scope management, Project management), Tools (stubs for now), Operations (Finance, Time tracking). Each page is markdown in src/content/wiki-v2/ with frontmatter (title, section, subsection, order). Adding a new page is dropping a file in. Voice is opinion-driven where v2 is directional, granular where it carries an SOP from the old wiki. Custom prose styling with numbered H2s via CSS counters, pull-quote and eyebrow-card blockquote variants picked structurally, GFM tables, GFM checklist styling, and a dedicated wiki shell with a secondary left rail (search, section list, active accent bar). PageTransition skips the fade for wiki routes so the rail stays mounted and navigation feels instant" },
      { type: "added", text: "Wiki v2 inline comments: highlight any text on any wiki page, drop a comment anchored to that selection, thread replies, resolve and re-open. Yellow underline marks open threads, muted grey marks resolved. Click a highlight to slide in a side panel with the thread. Author name stored per browser in localStorage so each person only types it once. Anchors store ~80 chars of context before and after the selection so they survive content edits via fuzzy re-locate. Backed by a new wiki_comments Supabase table (migration 022) with localStorage fallback per the project pattern" },
      { type: "improved", text: "Promoted Wiki v2 to be THE wiki. Sidebar 'Wiki' pinned link now points to /wiki-v2 instead of the legacy /tools/ops-wiki. Old wiki added to the Shelved list as 'Wiki (legacy)' so the route stays reachable for reference but isn't surfaced in nav. The new wiki is now the team's primary brain for both directional and operational knowledge" },
    ],
  },
  {
    id: "cl-77",
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "added", text: "Quick-add flow on /company/contracts. + New agreement button in the header opens a three-field modal: Name, Role, Compensation. Everything else defaults (contractor, today, monthly, GBP, generate both NDA and contract) so reuse is fast. If a Person with the exact name already exists the modal surfaces a notice and reuses that row (with a confirm) instead of duplicating. New Person rows are created on the fly so you don't need to pre-populate /company/people first" },
      { type: "added", text: "Onboarding flow preview at /onboarding-preview.html. Standalone HTML prototype (no auth, no Launchpad chrome) of the new slide-deck-style onboarding: 12 user steps covering brand basics, customer + competitors, project specifics, goals, risk + bottlenecks, design vibes, design love/meh/avoid rating, voice, app stack picker + per-app keep/replace verdict, access + assets + file upload, review summary. Auto-saves to localStorage. Includes all fields from the current production /onboard form plus five new visual sections (email capture, design rating, app stack picker + deep dive, copy examples, design inspiration brands). Lives in /public so Vercel serves it directly; not yet wired to Supabase. Preview only" },
      { type: "improved", text: "Onboarding preview welcome slide tones down the pitch energy: headline is just 'Onboarding', drops the marketing meta tiles, keeps a single utility lede. Top-left now uses the actual el-logo.svg instead of a placeholder dot + text badge so the page feels like an Ecom Landers tool rather than a generic form" },
    ],
  },
  {
    id: "cl-75",
