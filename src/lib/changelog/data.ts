...
const seedChangelog: ChangelogEntry[] = [
  {
    ...
    changes: [
      { type: "fixed", text: "/me/invoices now also matches Person by full_name as a third fallback (after pod_member_id + email). Covers cases where the user's AppUser row has email blank or stale. Also logs a console breadcrumb when matching fails, and the unlinked screen now shows the signed-in name + email so the admin can spot the mismatch fast" },
