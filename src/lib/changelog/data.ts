...
const seedChangelog: ChangelogEntry[] = [
  {
    ...
    changes: [
      ...
      { type: "added", text: "Pods and templates now sync across the whole team, not just the browser you edited them in. Custom pods, moves between pods, and template edits are stored in Supabase and shared, so a teammate sees the same structure you do." },
  ...
  },
  {
    ...
    date: "17 July 2026",
    version: "3.2.0",
    title: "One schedule, one owner, one checklist: the delivery board stops contradicting itself",
    changes: [
      { type: "added", text: "The build schedule now lives on the card, typed by the PM. One date per column, no computation: a card sitting in Design compares itself to its own Design date and goes red once it passes. The card totals the span, adds a 3-day buffer and tells you what to quote the client (\"12 days, tell the client 15\"), and flags it when the plan runs longer than the build was sold for. Undated columns read neutral rather than late, so a half-planned card doesn't flood the board red. Stored per card in kanban_tasks.phase_deadlines (migration 057)" },
      { type: "removed", text: "Both deadline cascades, and the disagreement between them. Deadlines used to be derived in TWO places from the same two inputs: PHASE_DAYS_BY_TURNAROUND + phaseInternalDueDate() dated each column, while SUBTASK_TEMPLATE.days + subtaskDeadline() dated each checklist step. Design was day 4 in one and day 3 in the other, and nothing reconciled them, so the card face and the checklist could show different dates for the same work. A schedule guessed from a day budget was never as good as the one the PM already knows, so both are gone (~180 lines)" },
      { type: "removed", text: "Phase 1 / Phase 2 deadline and Client approved as typed fields on the card. Phase 1 is just the External Revisions date and Phase 2 is the Launch date, so they're read off the schedule and shown as quiet markers instead of asking for the same answer twice. Client approved had no readers left once the Phase 2 anchor went (isClientApproved derives from phase history). Start date remains, marked as project-level since it's shared by every card on the project" },
      { type: "improved", text: "The card checklist is a flat four: Brief provided, Desktop Viewports Created, Development Handover Complete, Launch Ready and Test Setup. The other nine steps either restated the column the card was already in (Initial Design, Internal Revisions, Client Revisions, Development, Internal QA, Client Approval) or belonged to phases that left for the Results Engine. Ticking a box to say what the board already said was double-entry: two places to say the same thing, and two places to disagree. What survives is what the board can't see, the artifacts and the gates. The four phase tabs went with them; steps still lock by phase, and a locked row now says which (\"unlocks in Development\") instead of always claiming it waits on earlier steps" },
      { type: "improved", text: "The design handover reads as what it is: a checklist row with the action on the right, wearing its own step name, rather than a panel bolted into the middle of the list" },
      { type: "fixed", text: "The card face and the card modal could show different checklists. Only the modal normalised its subtasks, so a card that hadn't been opened since the change still showed the retired steps on its face. Both now read one checklistFor(d) resolver, which also prunes retired steps off older cards while keeping anything a PM added by hand. Tickets and Documents no longer get a build checklist at all, and audits get the brief only" },
      { type: "fixed", text: "Three surfaces read the raw dueDate the schedule replaced, so a scheduled card lost its date in each: the board's due sort dropped it to the bottom as if dated year 9999, KPI's on-time percentage silently stopped counting it, and My Tasks showed \"on track\" while the board showed the same card red. All three now resolve through cardDueDate(), so a build's date is its Launch column and My Tasks and the board agree" },
      { type: "fixed", text: "The project dates were unreachable on any card that needed them. The editor only rendered once a start date already existed, but it was the only place a start date could be SET, so a build stranded on \"TBC\" could never be dated. It was also gated on projectType === \"build\" while the type is optional in the data and the convention everywhere else (roster.ts) is that anything not explicitly \"retainer\" IS a build, so the block was hidden on every project whose type was never set" },
      { type: "added", text: "Migration 058: the client_csm table. src/lib/client-csm/store.ts has been reading and writing it since the /clients repoint, but the table was never created, so every CSM entry (renewal date, MRR, cadence checklist) silently fell back to localStorage. The numbers lived in one browser, invisible to everyone else and gone with the cache" },
    ],
  },
  {
    id: "cl-140",
  ...
  },
  {
    ...
    changes: [
      { type: "fixed", text: "/me/invoices Person lookup now routes through a new /api/me/resolve-person endpoint that uses the Supabase service role to read company_people, bypassing RLS that was hiding rows from team-role users. The endpoint matches on pod_member_id → email → full_name in priority order. Unlinked screen also shows the signed-in name + email so admins can spot mismatches fast" },
      { type: "fixed", text: "/me/invoices submit + past-list also bypass RLS via new /api/me/submit-invoice + /api/me/my-invoices endpoints. Team members can now submit invoices, see them in /finance, and see their own past submissions in the list below the form (previously blocked even after the Person resolver landed). Submit endpoint validates the linked_person_id against company_people before inserting" },
      { type: "improved", text: "/me/invoices now has the real drag-and-drop PDF upload (instead of the file-URL text field). Uploads land in the finance-documents bucket via /api/team-invoice/upload, and the signed URL + filename auto-populate on the invoice row. Submit Invoice entry points across the sidebar, team sidebar, team tools nav, and /team/payments all point at /me/invoices so there's one unified flow: upload PDF, tied to your Person, see your past submissions" },
      { type: "added", text: "Submit Invoice as a top-level item in the main sidebar, sitting just below Team Tools. Visible to everyone (admins included). Also in the ⌘K command palette so it's one shortcut away from anywhere" },
      { type: "added", text: "Member vs Admin access levels. Members (team role) now see Delivery (the kanban board) alongside My Tasks, Hero Offer, Training and tools - so fulfilment/delivery people like Alister aren't locked out. Admins keep everything (Onboarding, Old Delivery, KPIs, Sales, Retention, Finance, Admin). Set someone's level on their Person page via a standalone Member / Admin toggle (separate from login credentials - no password reset needed to promote/demote), or on the Team Access page (⌘K → 'team access'). Role changes apply on the person's next sign-in. Backed by a service-role /api/admin/set-user-role endpoint so RLS doesn't block it. Password resets never touch the access level" },
      { type: "fixed", text: "Members couldn't reach Delivery - /kanban wasn't in the team allowed-paths so they got bounced to /me. Now members get the full delivery board" },
      { type: "added", text: "My Tasks 'View as' picker now works for admin/cro on their own logins, not just shared-password sessions. A lead like Archie (Head of Development) can flip to any team member - e.g. Angel - to see their tasks, then back to their own. Defaults to their own tasks; the picked person drives both the board and the title. Members stay locked to their own assigned work (no snooping). Note: the person needs Admin or CRO access for the picker to show" },
      { type: "fixed", text: "Members no longer see the 'View as' dropdown on My Tasks at all (previously it showed for members without a linked pod member, even though they couldn't use it). The picker is admin/cro only now" },
      { type: "improved", text: "Members land straight on My Tasks after sign-in instead of the /me card-grid hub. /me (profile, password, invoice) still reachable by URL + the Submit Invoice sidebar item" },
      { type: "added", text: "Members get a read-only Delivery board. They can open any card and add content (notes, Figma URL, strategy brief, screenshots) but can't drag cards between columns, change due dates, tick things through phases (complete / approve / request revisions / QA / set live / conclude), add or delete cards, add or delete projects, or reassign pods. That project-management layer stays with admin/cro. Driven by role - promote someone to Admin to give them full control" },
      { type: "added", text: "Client Success OS concept preview at /preview/client-success (mock data, not wired). Two engines: an Account Playbook that runs the same touchpoint cadence for every client regardless of tier (weekly update, day-30 review, monthly report, day-75 refresh, day-90 close, upsell, QBR - each dated + owned + linked to its Hero Offer deck), and a Commitments board that puts an owner + SLA on every off-call promise so nothing evaporates. Plus a cross-client ops-review roll-up. Would live inside Retention" },
      { type: "removed", text: "Email-link sign-in option from the login page. Email + password and Admin access are the only entry points now. Cleans up the bottom row of the sign-in card" },
  ...
  },
  {
    ...
    changes: [
      { type: "improved", text: "Add Person now defaults to Contractor (the agency runs on contractors, not employees). Saves a click on every add. Switch to Employee for founders / full-time hires" },
