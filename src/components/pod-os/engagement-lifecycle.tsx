...
export function EngagementLifecycle({
    ...
    return (
      <div>
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className="text-[11px] text-subtle">points → bucket → duration</span>
        </div>
        <div className="grid grid-cols-3 gap-1.5">
          {SPRINT_PHASES.map((p, i) => (
            <div key={i} className="rounded-lg border border-border bg-background px-2.5 py-2">
              ...
              <div className="mt-0.5 text-[10px] text-subtle">{p.hint}</div>
            </div>
          ))}
        </div>
        {start && (
          <div className="mt-2 text-[11px] text-subtle">Kickoff {fmtDayMonth(start)} (§1.4)</div>
        )}
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {/* window track */}
      <div className="relative h-7 w-full overflow-hidden rounded-lg border border-border bg-background">
      ...
      <div className="relative mt-1 h-4">
        {[
          ...
          <div
            key={m.d}
            className="absolute -translate-x-1/2 text-[9px] text-subtle"
            style={{ left: `${pctOf(m.d)}%` }}
          >
            <span className="block h-1.5 w-px bg-muted" style={{ marginLeft: "50%" }} />
