  ...
  return (
    <div
      className={`
        ...
        ${selected ? "border-success shadow-lg" : "border-success/20 shadow-sm hover:shadow-md"}
      `}
    >
      <Handle type="target" position={Position.Left} className="!w-2.5 !h-2.5 !bg-success !border-2 !border-surface" />

      {/* Header */}
      <div className="flex items-center justify-between px-3.5 py-2 border-b border-success/15">
      ...
      <div className="px-3.5 py-3">
        ...
        {/* Metrics */}
        {optInCvr != null && (
          <div className="mt-2 pt-2 border-t border-success/15">
            <p className="text-[8px] text-muted uppercase tracking-wider mb-0.5">Opt-in CVR</p>
            <p className={`text-xs font-semibold ${optInCvr >= 25 ? "text-success" : optInCvr < 10 ? "text-danger" : "text-foreground"}`}>
              {optInCvr}%
            </p>
          </div>
        )}
        ...
        {done > 0 && (
          <div className="mt-2 flex items-center gap-1.5">
            <div className="flex-1 h-1 bg-success/15 rounded-full overflow-hidden">
              <div className="h-full bg-success rounded-full" style={{ width: `${(done / total) * 100}%` }} />
            </div>
        ...
        {data.previewUrl && (
          <a href={data.previewUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
            className="inline-flex items-center gap-1 mt-2.5 px-2 py-1 text-[10px] font-medium text-success bg-success/10 rounded-md hover:bg-success/20 transition-colors">
      ...
      <Handle type="source" position={Position.Right} className="!w-2.5 !h-2.5 !bg-success !border-2 !border-surface" />
    </div>
  );
}
