    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {canEdit && (
            <button
              ...
              className="text-[11px] text-subtle hover:text-foreground underline-offset-2 hover:underline"
      ...
      <section className="rounded-xl border border-border bg-surface p-4 shadow-[var(--shadow-soft)]">
        ...
        {agent.tools.length === 0 ? (
          <p className="text-sm text-subtle">No tools configured.</p>
        ) : (
          <div className="flex flex-wrap gap-1.5">
            {agent.tools.map((t) => (
              <span
                key={t}
                className="font-mono text-[11px] px-2 py-1 rounded-md bg-surface-raised text-foreground border border-border"
              >
                {t}
              </span>
            ))}
