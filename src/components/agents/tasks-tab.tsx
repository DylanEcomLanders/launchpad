  ...
  return (
    <div className="rounded-xl border border-[#E5E5EA] bg-white shadow-[var(--shadow-soft)]">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-[#E5E5EA] p-3">
            ...
            return (
              <button
                ...
                className={`rounded-md px-2 py-1 text-[11px] font-medium transition-colors ${
                  active ? "bg-[#1B1B1B] text-white" : "text-[#7A7A7A] hover:text-[#1B1B1B]"
                }`}
              >
                {label}
              </button>
        ...
        <div className="ml-auto flex items-center gap-2 text-[11px] text-[#7A7A7A]">
          <span>Sort:</span>
          <select
            value={sort}
            onChange={(e) => setSort(e.target.value as SortKey)}
            className="rounded-md border border-[#E5E5EA] bg-white px-2 py-1 text-[11px] focus:outline-none focus:border-[#1B1B1B]"
          >
      ...
      {/* Empty state */}
      {visible.length === 0 ? (
        <div className="p-12 text-center text-sm text-[#7A7A7A]">
          {tasks.length === 0
            ...
            return (
              <li key={task.id}>
                <button
                  ...
                  className="grid w-full grid-cols-[110px_1fr_120px_60px] items-center gap-3 px-4 py-2.5 text-left hover:bg-[#F7F8FA] transition-colors"
                >
                  <span
                  ...
                  <span className="truncate text-sm text-[#1B1B1B]">{task.input}</span>
                  ...
                  <span className="text-[11px] tabular-nums text-[#A0A0A0] text-right">
                    {durationLabel(task)}
                  </span>
                </button>
                {isOpen && (
                  <div className="border-t border-[#E5E5EA] bg-[#F7F8FA] px-4 py-3 space-y-2">
...
function DetailRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="grid grid-cols-[80px_1fr] gap-3 text-xs">
      ...
      <span className={mono ? "font-mono text-[#1B1B1B]" : "text-[#1B1B1B] whitespace-pre-wrap"}>{value}</span>
    </div>
  );
}
