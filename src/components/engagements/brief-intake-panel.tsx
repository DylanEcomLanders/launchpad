        ...
        <div className="flex items-center gap-3">
          {intake && (
            <Link
              ...
              className="text-[11px] text-[#666] hover:text-[#1B1B1B] inline-flex items-center gap-1"
              title="Open in inbox"
            >
          ...
          <button
            onClick={onToggle}
            className="text-[#999] hover:text-[#1B1B1B]"
...
function IntakeBody({ intake }: { intake: OnboardingSubmission }) {
  return (
    <div className="px-4 pb-4 pt-1 border-t border-[#E5E5EA] grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            {[
              ...
              return (
                <li key={key} className="text-[12px] flex items-center gap-2">
                  <span className={`inline-block size-3 rounded-full border ${checked ? "bg-[#1B5E20] border-[#1B5E20]" : "bg-white border-[#D4D4D8]"}`} />
                  <span className={checked ? "text-[#1B1B1B]" : "text-[#999]"}>{label}</span>
                </li>
              );
            })}
          </ul>
        ) : (
          <p className="text-[12px] text-[#999]">No PM checklist captured.</p>
        )}
        {intake.pm_notes && (
          <p className="text-[12px] text-[#444] mt-3 italic whitespace-pre-wrap">{intake.pm_notes}</p>
        )}
      </IntakeGroup>
            ...
            {intake.uploaded_files?.map((f, i) => (
              <li key={i}>
                <a
                  ...
                  className="flex items-center gap-2 px-3 py-2 bg-white border border-[#E5E5EA] rounded text-[12px] text-[#444] hover:border-[#1B1B1B] transition-colors"
                >
                  <DocumentArrowUpIcon className="size-3.5 text-[#999] shrink-0" />
      ...
      {intake.additional_info && (
        <IntakeGroup title="Additional info" wide>
          <p className="text-[12px] text-[#444] whitespace-pre-wrap">{intake.additional_info}</p>
        </IntakeGroup>
      )}
    </div>
  );
  ...
  return (
    <div className="px-4 pb-4 pt-1 border-t border-[#E5E5EA]">
              ...
              {populated.map((field) => {
                ...
                return (
                  <div key={field.key}>
                    ...
                    {isUrl ? (
                      <a href={value} target="_blank" rel="noopener noreferrer" className="text-[12px] text-[#1B1B1B] hover:underline break-all">
                        {value}
                      </a>
                    ) : (
                      <p className="text-[12px] text-[#1B1B1B] whitespace-pre-line leading-snug">{value}</p>
                    )}
                  </div>
                );
              })}
      ...
      {!brief.primaryGoal && !brief.primaryContact && (
        <p className="text-[12px] text-[#999] italic mt-3">No brief captured. Link an onboarding submission to populate this panel.</p>
      )}
    </div>
  );
}
...
function IntakeField({
  ...
  return (
    <div>
      <p className="text-[10px] font-medium text-[#999]">{label}</p>
      {link ? (
        <a
          href={value}
          target="_blank"
        ...
        <p className="text-[12px] text-[#444] whitespace-pre-wrap break-words">{value}</p>
      )}
    </div>
  );
}
