  ...
  return (
    <div className="bg-[#FAFAFB] border border-[#EDEDEF] rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs font-semibold text-[#1B1B1B]">Sync from Figma</div>
          <div className="text-[10px] text-[#A0A0A0] mt-0.5">
            Paste a frame URL — slices auto-generate. Replaces existing.
          </div>
        </div>
        <div className="text-[10px] text-[#7A7A7A] tabular-nums">
      ...
      {progress && (
        <div className="text-[11px] text-[#1B1B1B] bg-white border border-[#E5E5EA] rounded px-3 py-2">
          {progress}
        </div>
      )}
      {error && (
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#1B1B1B] text-white text-xs font-semibold rounded-lg hover:bg-[#2D2D2D] disabled:opacity-40 transition-colors"
      >
