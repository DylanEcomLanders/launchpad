  ...
  return (
    ...
    >
      <div
        ...
        onClick={onClose}
      />
      <div className="relative w-full max-w-[640px] bg-white rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] overflow-hidden">
        {/* Search input */}
        <div className="flex items-center gap-3 px-4 h-12 border-b border-[#EDEDEF]">
          <MagnifyingGlassIcon className="size-4 text-[#A0A0A0]" />
          <input
            ref={inputRef}
            value={query}
            onChange={(e) => {
            ...
            placeholder="Jump to..."
            className="flex-1 bg-transparent outline-none text-[14px] text-[#1B1B1B] placeholder:text-[#A0A0A0]"
          />
          <button
            onClick={onClose}
        ...
        >
          {grouped.length === 0 ? (
            <div className="px-4 py-10 text-center text-[13px] text-[#A0A0A0]">
              No matches for &ldquo;{query}&rdquo;
            </div>
          ) : (
            grouped.map((g) => (
              <div key={g.name} className="mb-1">
                <div className="px-4 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-wider text-[#A0A0A0]">
                  {g.name}
                </div>
                {g.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-4 py-2 text-left text-[13px] transition-colors ${
                        active
                          ? "bg-[#F3F3F5] text-[#1B1B1B]"
                          : "text-[#3A3A3A] hover:bg-[#F7F8FA]"
                      }`}
                    >
                      {item.icon && (
                        <span className="text-[#7A7A7A] shrink-0">{item.icon}</span>
                      )}
                      <span className="flex-1 truncate">{item.label}</span>
                      {item.shelved && (
                        <span className="text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded bg-[#F0F0F0] text-[#999]">
                          Shelved
                        </span>
                      )}
        ...
        {/* Footer hints */}
        <div className="flex items-center justify-between px-4 h-9 border-t border-[#EDEDEF] bg-[#FAFAFB] text-[11px] text-[#A0A0A0]">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-white border border-[#E5E5EA] font-mono text-[10px]">↑↓</kbd>
              Navigate
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-white border border-[#E5E5EA] font-mono text-[10px]">↵</kbd>
            ...
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-white border border-[#E5E5EA] font-mono text-[10px]">esc</kbd>
              Close
            </span>
          </div>
