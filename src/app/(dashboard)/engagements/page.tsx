...
export default function EngagementsPage() {
  ...
  return (
    <div className="px-6 py-6 max-w-[1400px] mx-auto">
      <header className="mb-6 border-b border-border pb-4 flex items-start justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Link
            ...
            className="inline-flex items-center gap-1.5 text-[12px] font-medium text-muted hover:text-foreground hover:bg-surface-raised px-3 py-2 rounded"
          >
      ...
      {allEngagements.length === 0 ? (
        <section className="rounded-lg border border-dashed border-border bg-background p-12 text-center">
          ...
          <p className="text-[12px] text-muted max-w-md mx-auto mb-4">
      ...
      <div className="overflow-x-auto rounded-lg border border-border bg-surface">
        <table className="w-full min-w-[760px] text-left text-[13px]">
          <thead>
            <tr className="border-b border-border text-[10px] font-semibold uppercase tracking-wider text-subtle">
                  ...
                  >
                    <td className="px-4 py-3">
                      <span className="inline-flex items-center gap-2">
                        ...
                        <span className="font-semibold text-foreground">{eng.brand}</span>
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-3 text-muted tabular-nums">Pod {eng.podNumber}</td>
                    <td className="px-3 py-3 text-muted tabular-nums">
                      {where}
                      <span className="ml-1 text-muted">· {health.done}/{health.total}</span>
                    </td>
                    <td className="px-3 py-3">
                      {next ? (
                        <span className="text-foreground">
                          <span className="truncate">{next.name}</span>
                          <span className={`ml-1.5 text-[11px] tabular-nums ${next.dueInDays < 0 ? "font-semibold text-danger" : "text-subtle"}`}>
