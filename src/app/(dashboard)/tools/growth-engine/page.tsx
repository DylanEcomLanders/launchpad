...
export default function GrowthEnginePage() {
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="border-b border-border bg-surface px-6 py-0 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-sm font-semibold text-foreground">Agency Funnels</h1>
          </div>

          {/* Right: View switcher + stats */}
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 bg-surface-raised rounded-lg p-0.5">
              <button onClick={() => setView("funnel")} className={`px-3 py-1 text-[11px] font-medium rounded-md transition-colors ${view === "funnel" ? "bg-surface text-foreground shadow-sm" : "text-subtle"}`}>Funnel</button>
              <button onClick={() => setView("gaps")} className={`px-3 py-1 text-[11px] font-medium rounded-md transition-colors ${view === "gaps" ? "bg-surface text-foreground shadow-sm" : "text-subtle"}`}>Gap Grid</button>
            </div>
            <span className="text-[10px] text-muted">{totalFlows} funnels · {totalNodes} nodes</span>
          </div>
        </div>
      ...
      {view === "gaps" && (
        <div className="flex-1 overflow-auto p-6">
          <div className="max-w-5xl mx-auto">
            <div className="mb-6">
              <h2 className="text-lg font-bold text-foreground">Gap Analysis: {PROFILES.find(p => p.key === profile)?.label}</h2>
              <p className="text-xs text-muted mt-1">Red = missing, Amber = planned, Green = live</p>
            </div>
            <div className="border border-border rounded-xl overflow-hidden bg-surface">
              {/* Header row */}
              <div className="grid grid-cols-[140px_1fr_1fr_1fr_1fr] border-b border-border">
                <div className="px-4 py-3 bg-background" />
                {STAGES.map(s => (
                  <div key={s.key} className="px-4 py-3 bg-background border-l border-border">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted">{s.label}</p>
                    <p className="text-[9px] text-muted">{s.warmth}</p>
                  </div>
                ))}
              </div>
              {/* Channel rows */}
              {PLATFORMS.map(ch => {
                return (
                  <div key={ch.key} className="grid grid-cols-[140px_1fr_1fr_1fr_1fr] border-b border-border last:border-0">
            ...
            {(() => {
              ...
              return (
                <p className="text-xs text-muted mt-4">{gaps} gap{gaps !== 1 ? "s" : ""} identified across {PLATFORMS.length} channels</p>
              );
            })()}
          </div>
        </div>
        ...
        <div className="w-64 border-r border-border bg-surface overflow-y-auto shrink-0">
          {selectedNode ? (
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted">Edit Node</p>
                <button onClick={() => setSelectedNode(null)} className="text-[10px] text-muted hover:text-foreground">Done</button>
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm px-2 py-1.5 border border-border rounded"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full text-xs px-2 py-1.5 border border-border rounded min-h-[60px] resize-y"
                />
              </div>
              <div>
              ...
              <div>
                <label className="text-[10px] text-muted block mb-1">Funnel Stage</label>
                <select
              ...
              {selectedNode.type === "trafficNode" && (
                <div>
                  <label className="text-[10px] text-muted block mb-1">Warmth</label>
                  <select
              ...
              {selectedNode.type === "leadMagnetNode" && (
                <div>
                  <label className="text-[10px] text-muted block mb-1">Format</label>
                  <select
              ...
              {selectedNode.type === "emailSequenceNode" && (
                <div className="space-y-2">
                  <div>
                    <label className="text-[10px] text-muted block mb-1">Email Count</label>
                  ...
                  <div>
                    <label className="text-[10px] text-muted block mb-1">Open Rate %</label>
                  ...
                  <div>
                    <label className="text-[10px] text-muted block mb-1">Click Rate %</label>
              ...
              {(selectedNode.type === "pageNode" || selectedNode.type === "leadMagnetNode") && (
                <div className="pt-3 border-t border-border">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-muted mb-2">Content Checklist</p>
                  {(["headline", "hook", "offer", "cta", "socialProof"] as const).map((slot) => {
                    ...
                    return (
                      <label key={slot} className="flex items-center gap-2 py-1 cursor-pointer">
                        ...
                        <span className={`text-xs ${checked ? "text-foreground" : "text-subtle"}`}>{labels[slot]}</span>
                      </label>
                    );
                  })}
                </div>
              ...
              <div>
                <label className="text-[10px] text-muted block mb-1">Preview URL</label>
                <input
                  type="url"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full text-xs px-2 py-1.5 border border-border rounded min-h-[80px]"
                ...
                {trafficSources.map((src) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center gap-2 px-3 py-2 border border-border rounded-lg cursor-grab hover:border-subtle hover:shadow-sm transition-all"
                    >
