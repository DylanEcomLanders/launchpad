...
export default function CroAuditPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 md:px-12 py-12 md:py-16">
      {/* Header */}
      <div className="mb-10">
        <h1 className="text-[28px] leading-tight font-bold text-foreground mb-1">CRO Audit</h1>
              ...
              <div className="space-y-3">
                {result.quickWins.map((win, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <span className="flex-shrink-0 size-5 rounded-full bg-white text-background text-[10px] font-bold flex items-center justify-center mt-0.5">
                      {i + 1}
                    </span>
...
function ImageUpload({
  ...
  return (
    <div
      ...
      `}
    >
      <PhotoIcon className="size-8 text-muted" />
      <div className="text-center">
        <p className="text-sm font-medium text-subtle">{label}</p>
        <p className="text-xs text-subtle mt-0.5">{sublabel}</p>
      </div>
