  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-[#0F0F10] rounded-2xl p-5 ring-1 ring-white/[0.04]">
        <h2 className="text-sm font-semibold text-[#E5E5EA] mb-3">Generate a report</h2>
        <div className="flex flex-col sm:flex-row items-end gap-3">
          <div className="flex-1">
            <label className="text-[10px] uppercase tracking-wider text-[#71757D] font-semibold">Client</label>
      ...
      {!hydrated ? (
        ...
        <div className="bg-[#0F0F10] rounded-2xl p-12 text-center ring-1 ring-white/[0.04]">
          <p className="text-sm text-[#71757D]">No reports yet. Generate one above.</p>
        </div>
      ) : (
        <ul className="space-y-2">
          {reports.map((r) => (
            <li key={r.id}>
              <Link href={`/tools/reports/${r.id}`} className="block bg-[#0F0F10] rounded-xl p-4 ring-1 ring-white/[0.04] hover:ring-sky-500/30 transition-all">
                <div className="flex items-center gap-4">
                  ...
                  {r.status !== "draft" && (
                    <a href={`/report-output/${r.output_slug}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="inline-flex items-center gap-1 text-[11px] uppercase tracking-wider text-[#71757D] hover:text-sky-300 shrink-0">
                      View
