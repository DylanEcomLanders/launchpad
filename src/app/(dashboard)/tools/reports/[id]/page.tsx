...
export default function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (notFound || !report) return (<div className="p-6"><div className="bg-background rounded-2xl p-8 text-center ring-1 ring-white/[0.04]"><p className="text-sm text-subtle mb-3">Report not found.</p><Link href="/tools/reports" className="text-[12px] uppercase tracking-wider text-sky-300">← Back</Link></div></div>);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={deleteReport} className="p-1.5 rounded-md text-subtle hover:text-rose-400"><TrashIcon className="size-4" /></button>
        </div>
      </div>
...
function Stat({ label, value, accent = "default" }: { label: string; value: number; accent?: "default" | "emerald" | "zinc" }) {
  const tint = accent === "emerald" ? "text-emerald-300" : accent === "zinc" ? "text-zinc-300" : "text-foreground";
  return (
