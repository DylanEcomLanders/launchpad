...
export default function ReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (notFound || !report) return (<div className="p-6"><div className="bg-surface rounded-2xl p-8 text-center border border-border"><p className="text-sm text-subtle mb-3">Report not found.</p><Link href="/tools/reports" className="text-[12px] uppercase tracking-wider text-info">← Back</Link></div></div>);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <Link href="/tools/reports" className="inline-flex items-center gap-1 text-[11px] uppercase tracking-wider text-subtle hover:text-foreground mb-3">
          ...
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            <div className="size-8 rounded-xl bg-surface-raised border border-border flex items-center justify-center shrink-0">
              <ChartPieIcon className="size-4 text-foreground" />
            </div>
          ...
          <div className="text-[12px] text-subtle flex items-center gap-2">
            ...
            <span>·</span>
            <Link href={sharePath} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-info">
              Open report
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={deleteReport} className="p-1.5 rounded-md text-subtle hover:text-danger"><TrashIcon className="size-4" /></button>
        </div>
      </div>
      ...
      <Section title={`Tests this period (${report.tests.length})`}>
        {report.tests.length === 0 ? (
          <p className="text-[11px] italic text-subtle">Nothing to summarise.</p>
        ) : (
          <ul className="space-y-2">
            {report.tests.map((t) => (
              <li key={t.test_id} className="bg-surface rounded-lg p-3 border border-border">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-surface text-muted">{t.status}</span>
                  {t.outcome && <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-success/10 text-success">{t.outcome}{t.uplift_pct !== undefined && ` ${t.uplift_pct >= 0 ? "+" : ""}${t.uplift_pct}%`}</span>}
                  {t.surface && <span className="text-[11px] text-subtle">{t.surface}</span>}
                </div>
                <p className="text-[13px] text-foreground line-clamp-2">{t.hypothesis || "(no hypothesis)"}</p>
              </li>
            ))}
          </ul>
        )}
...
function Stat({ label, value, accent = "default" }: { label: string; value: number; accent?: "default" | "emerald" | "zinc" }) {
  const tint = accent === "emerald" ? "text-success" : accent === "zinc" ? "text-muted" : "text-foreground";
  return (
