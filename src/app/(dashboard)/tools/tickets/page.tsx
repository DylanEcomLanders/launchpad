    ...
    <div className="max-w-4xl mx-auto py-10 px-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-[28px] leading-tight font-bold text-foreground">{showTrash ? "Trash" : "Tickets"}</h1>
            <button
              onClick={() => { setShowTrash(!showTrash); setConfirmDeleteId(null); }}
              className={`text-[10px] font-medium px-2.5 py-1 rounded-full transition-colors ${
                showTrash ? "bg-white text-background" : "bg-surface-raised text-muted hover:bg-border"
              }`}
            >
        ...
        <button
          ...
          className="text-[11px] font-medium text-muted border border-border px-3 py-1.5 rounded-lg hover:bg-surface-raised"
        >
      ...
      {showMappings && (
        <div className="border border-border rounded-xl bg-surface p-5 mb-6">
          <h3 className="text-xs font-semibold text-foreground mb-1">Channel → Client Mappings</h3>
          <p className="text-[10px] text-muted mb-4">Map Slack channel IDs to client names. Tickets logged from a channel auto-assign to the client.</p>

          {mappings.length > 0 && (
            <div className="space-y-1.5 mb-4">
              {mappings.map((m) => (
                <div key={m.channel_id} className="flex items-center justify-between py-1.5 px-3 bg-background rounded-lg">
                  <div className="flex items-center gap-3">
                    <code className="text-[10px] text-subtle font-mono">{m.channel_id}</code>
                    <span className="text-xs font-medium text-foreground">{m.client_name}</span>
                  </div>
                  <button
          ...
          <div className="flex items-end gap-2">
            ...
            <button
              ...
              className="px-3 py-2 bg-white text-background text-xs font-medium rounded-lg disabled:opacity-30"
            >
              Add
            </button>
          </div>
      ...
      <div className="flex items-center gap-4 mb-4">
          ...
          {[{ key: "all" as const, label: "All" }, ...TICKET_STATUSES].map((s) => (
            <button
              ...
              className={`px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider rounded-full transition-colors ${
                filterStatus === s.key ? "bg-white text-background" : "text-muted hover:text-foreground"
              }`}
            >
              {s.label}
            </button>
        ...
        {clients.length > 1 && (
          <select
            ...
            className="text-[10px] text-muted border border-border rounded px-2 py-1"
          >
      ...
      {loading ? (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-semibold text-foreground">{ticket.title}</p>
                  ...
                  <p className="text-[10px] text-subtle mt-0.5 line-clamp-1">{ticket.description}</p>
                ...
                <select
                  ...
                  className="text-[10px] text-muted border border-border rounded px-1.5 py-1 shrink-0 cursor-pointer"
        ...
        <div className="border border-dashed border-border rounded-xl p-12 text-center">
          ...
          <p className="text-xs text-muted mt-1">Tickets logged via /ticket in Slack will appear here</p>
        </div>
      )}
...
function TicketDetailModal({ ticket, team, onClose, onStatusChange, onSaveNotes, onSaveTicket }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-surface rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-3 border-b border-border">
          ...
          <button onClick={onClose} className="text-muted hover:text-foreground"><XMarkIcon className="size-5" /></button>
        </div>

        <div className="p-5 space-y-4">
          {/* Meta */}
          <div className="flex items-center gap-4 text-[10px] text-subtle">
            <span className="font-medium text-muted">{ticket.client_name}</span>
          ...
          {/* Description */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted mb-1">Description</p>
            <p className="text-xs text-muted leading-relaxed whitespace-pre-wrap">{ticket.description}</p>
          </div>
          ...
          {/* Status */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted mb-1">Status</p>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-[10px] font-semibold rounded-lg border transition-colors ${
                    ticket.status === s.key
                      ...
                      : "text-muted border-border hover:border-subtle"
                  }`}
          ...
          {/* Type */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted mb-1">Type</p>
            <div className="flex gap-1.5">
          ...
          {/* Assignee */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted mb-1">Assignee</p>
            <select
              value={ticket.assignee_id || ""}
              onChange={(e) => onSaveTicket({ ...ticket, assignee_id: e.target.value || undefined })}
              className="text-xs text-muted border border-border rounded-lg px-2.5 py-1.5 w-full"
            >
          ...
          {/* Internal notes */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted mb-1">Internal Notes</p>
            <textarea
              value={notes}
            ...
            {notes !== ticket.notes && (
              <button
                ...
                className="mt-2 px-3 py-1.5 bg-white text-background text-[10px] font-medium rounded-lg"
              >
                Save Notes
              </button>
            )}
