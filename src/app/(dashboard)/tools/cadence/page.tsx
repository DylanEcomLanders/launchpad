...
export default function CadencePage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      <header>
        <div className="flex items-center gap-3 mb-2">
          <div className="size-9 rounded-xl bg-surface-raised border border-border flex items-center justify-center">
            <ChatBubbleOvalLeftEllipsisIcon className="size-5 text-foreground" />
          </div>
      ...
      {!hydrated ? (
        <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-24 bg-surface rounded-xl animate-pulse" />)}</div>
      ) : rows.length === 0 ? (
        ...
        <ul className="space-y-3">
          {rows.map((r) => (
            <li key={r.client_name} className="bg-surface rounded-2xl border border-border overflow-hidden">
              <div className="p-4 flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-sm font-semibold text-foreground">{r.client_name}</span>
                    {r.risks.length === 0 && (
                      <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-semibold bg-success/10 text-success ring-1 ring-success/20 inline-flex items-center gap-1">
                        <CheckCircleIcon className="size-3" /> Healthy
                      </span>
                    )}
                    {r.risks.map((risk, i) => (
                      <span key={i} className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-semibold inline-flex items-center gap-1 ${risk.severity === "danger" ? "bg-danger/10 text-danger ring-1 ring-danger/20" : "bg-warning/10 text-warning ring-1 ring-warning/20"}`} title={risk.detail}>
                  ...
                  <div className="text-[12px] text-subtle">
                    {r.last_touch ? (
                      <>
                        ...
                        {r.last_touch.awaiting_reply && <span className="text-warning"> · awaiting reply</span>}
                      </>
                    ) : (
                      "No touches logged"
                    )}
                ...
                <button onClick={() => setLogging(r.client_name === logging ? null : r.client_name)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md text-[11px] font-semibold uppercase tracking-wider bg-foreground text-background hover:bg-muted shrink-0">
                  <PhoneIcon className="size-3.5" />
                  {logging === r.client_name ? "Cancel" : "Log touch"}
                </button>
              </div>
              {logging === r.client_name && (
                <div className="border-t border-border p-4 bg-surface-raised space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    <label className="inline-flex items-center gap-2 text-[11px] text-muted">
                      <input type="checkbox" checked={draft.awaiting_reply} onChange={(e) => setDraft((d) => ({ ...d, awaiting_reply: e.target.checked }))} className="accent-warning" />
                      Awaiting their reply
                    </label>
                    <button onClick={() => logTouch(r.client_name)} disabled={!draft.summary.trim()} className="inline-flex items-center gap-1 px-3 py-1.5 rounded text-[11px] font-semibold uppercase tracking-wider bg-foreground text-background hover:bg-muted disabled:opacity-40">
