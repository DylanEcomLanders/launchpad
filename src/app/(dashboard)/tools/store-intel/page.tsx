  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 md:px-12 py-12 md:py-16">
        ...
        {/* Mode Toggle */}
        <div className="flex gap-1 mb-6 bg-surface-raised border border-border rounded-lg p-1 w-fit">
          <button
        ...
        {/* Input Form */}
        <div className="bg-surface-raised border border-border rounded-lg p-5 mb-8">
          {mode === "store" ? (
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-5 py-2.5 bg-white text-background text-sm font-medium rounded-md hover:bg-foreground disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            {loading ? (
              <>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface-raised border border-border rounded-lg p-5">
        <div className="flex items-center gap-2 mb-1">
          <DocumentTextIcon className="size-4 text-subtle" />
          <span className="text-[10px] font-semibold uppercase tracking-wider text-subtle">{result.pageType}</span>
        </div>
        <h2 className="text-sm font-semibold mb-1">{result.title}</h2>
        <p className="text-xs text-subtle break-all">{result.pageUrl}</p>
            ...
            {result.elements.ctas.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {result.elements.ctas.map((cta, i) => (
                  <span key={i} className="inline-flex items-center px-2.5 py-1 bg-surface border border-border rounded text-xs text-subtle">
                    {cta}
                  </span>
                ))}
              </div>
            ) : (
              <p className="text-xs text-subtle">No clear CTAs detected</p>
            )}
          </div>

          <div className="bg-surface-raised border border-border rounded-lg p-5">
            <div className="flex items-center gap-2 mb-3">
              <SwatchIcon className="size-4 text-subtle" />
              <h3 className="text-xs font-semibold uppercase tracking-wider text-subtle">Tech Stack</h3>
            </div>
            <p className="text-sm font-medium mb-2">{result.theme}</p>
            {result.appsDetected.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {result.appsDetected.map((app) => (
                  <span key={app} className="inline-flex items-center px-2 py-0.5 bg-surface border border-border rounded text-[11px] text-subtle">
                    {app}
                  </span>
                ))}
              </div>
            ) : (
              <p className="text-xs text-subtle">No major apps detected</p>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface-raised border border-border rounded-lg p-5">
        <div className="flex items-center gap-2 mb-4">
          <BuildingStorefrontIcon className="size-4 text-subtle" />
          <h2 className="text-sm font-semibold">{result.brand}</h2>
          <span className="text-xs text-subtle">{result.storeUrl}</span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {result.productAnalysis && (
          <div className="bg-surface-raised border border-border rounded-lg p-5">
            <div className="flex items-center gap-2 mb-3">
              <CubeIcon className="size-4 text-subtle" />
              <h3 className="text-xs font-semibold uppercase tracking-wider text-subtle">Price Analysis</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div>
                <p className="text-[10px] text-subtle uppercase">Range</p>
                <p className="font-medium tabular-nums">
                  ${result.productAnalysis.priceRange.min.toFixed(0)} – ${result.productAnalysis.priceRange.max.toFixed(0)}
                </p>
              </div>
              <div>
                <p className="text-[10px] text-subtle uppercase">Median</p>
                <p className="font-medium tabular-nums">${result.productAnalysis.priceRange.median.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-[10px] text-subtle uppercase">Discounted</p>
                <p className="font-medium tabular-nums">{result.productAnalysis.discounting.percent}</p>
              </div>
              <div>
                <p className="text-[10px] text-subtle uppercase">Avg Discount</p>
...
function SummaryCard({ summary }: { summary: string }) {
  return (
    <div className="bg-surface-raised border border-border rounded-lg p-5">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-subtle mb-2">Executive Summary</p>
...
function FindingsSummaryStrip({ findings }: { findings: Finding[] }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 bg-surface-raised border border-border rounded-lg px-5 py-3">
    ...
    <div className="space-y-4">
      {CATEGORY_ORDER.map((cat) => {
        ...
        return (
          <div
            key={cat}
            className={`bg-surface border border-border border-l-4 ${meta.border} rounded-lg overflow-hidden`}
          >
            {/* Category Header */}
            <button
              onClick={() => toggle(cat)}
  ...
  return (
    <div className="bg-surface-raised border border-border rounded-md p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        {finding.page && (
          <span className="text-[10px] px-2 py-0.5 bg-border rounded text-subtle">
            {finding.page}
          </span>
        )}
      </div>
      <h4 className="text-sm font-semibold text-foreground mb-1">{finding.title}</h4>
      <p className="text-xs text-subtle leading-relaxed">{finding.description}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-3 pt-2">
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-surface-raised border border-border rounded-md text-xs font-medium text-subtle hover:bg-border transition-colors"
      >
