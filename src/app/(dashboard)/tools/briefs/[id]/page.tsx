...
export default function BriefDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (notFound || !brief) return (<div className="p-6"><div className="bg-[#0F0F10] rounded-2xl p-8 text-center ring-1 ring-white/[0.04]"><p className="text-sm text-[#71757D] mb-3">Brief not found.</p><Link href="/tools/briefs" className="text-[12px] uppercase tracking-wider text-cyan-300 hover:text-cyan-200">← Back</Link></div></div>);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <Link href="/tools/briefs" className="inline-flex items-center gap-1 text-[11px] uppercase tracking-wider text-[#71757D] hover:text-[#E5E5EA] mb-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={deleteBrief} className="p-1.5 rounded-md text-[#71757D] hover:text-rose-400 hover:bg-rose-500/[0.1]"><TrashIcon className="size-4" /></button>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2 items-end">
              ...
              <div className="bg-emerald-500/15 ring-1 ring-emerald-500/30 rounded-md p-2 text-center">
                <div className="text-[9px] uppercase tracking-wider text-[#71757D]">Score</div>
...
function ScoreField({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (<div><div className="text-[9px] uppercase tracking-wider text-[#71757D] mb-1">{label}</div><input type="number" min={1} max={10} value={value} onChange={(e) => onChange(Math.max(1, Math.min(10, Number(e.target.value) || 1)))} className={`${inputClass} h-9 text-center`} /></div>);
}
