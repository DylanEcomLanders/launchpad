...
export default function MilestoneDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hydrated) return (<div className="p-6 space-y-3 max-w-5xl mx-auto">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-32 bg-background rounded-xl animate-pulse" />)}</div>);
  if (notFound || !m) return (<div className="p-6"><div className="bg-background rounded-2xl p-8 text-center ring-1 ring-white/[0.04]"><p className="text-sm text-subtle mb-3">Milestone not found.</p><Link href="/tools/lifecycle" className="text-[12px] uppercase tracking-wider text-sky-300">← Back</Link></div></div>);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={deleteMilestone} className="p-1.5 rounded-md text-subtle hover:text-rose-400"><TrashIcon className="size-4" /></button>
        </div>
      </div>
        ...
        <ul className="space-y-2">
          {m.items.map((item) => (
            <li key={item.id} className={`bg-black/40 rounded-xl p-3 ring-1 flex items-start gap-3 ${item.done_at ? "ring-emerald-500/20" : "ring-white/[0.04]"}`}>
              ...
              <button onClick={() => removeItem(item.id)} className="p-1 text-subtle hover:text-rose-400"><TrashIcon className="size-3.5" /></button>
            </li>
          ))}
        </ul>
