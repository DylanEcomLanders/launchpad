...
export default function MilestoneDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hydrated) return (<div className="p-6 space-y-3 max-w-5xl mx-auto">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-32 bg-surface rounded-xl animate-pulse" />)}</div>);
  if (notFound || !m) return (<div className="p-6"><div className="bg-surface rounded-2xl p-8 text-center border border-border"><p className="text-sm text-subtle mb-3">Milestone not found.</p><Link href="/tools/lifecycle" className="text-[12px] uppercase tracking-wider text-info">← Back</Link></div></div>);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <Link href="/tools/lifecycle" className="inline-flex items-center gap-1 text-[11px] uppercase tracking-wider text-subtle hover:text-foreground mb-3">
            <ArrowLeftIcon className="size-3.5" />
            All lifecycle
          </Link>
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            <div className="size-10 rounded-xl bg-surface-raised border border-border flex flex-col items-center justify-center text-center shrink-0">
              <div className="text-sm font-bold text-foreground leading-none">{m.day}</div>
            </div>
            <h1 className="text-2xl font-semibold text-foreground truncate">{m.client_name}</h1>
            <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-semibold ${STATUS_TINT[status]}`}>
              {STATUS_LABEL[status]}
            </span>
          </div>
          <div className="text-[12px] text-muted">{MILESTONE_TITLE[m.day]}</div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={deleteMilestone} className="p-1.5 rounded-md text-subtle hover:text-danger"><TrashIcon className="size-4" /></button>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-2xl p-5 border border-border">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-[11px] font-mono text-muted">{done} / {m.items.length}</div>
        </div>
      ...
      <div className="bg-surface rounded-2xl p-5 border border-border">
        ...
        <ul className="space-y-2">
          {m.items.map((item) => (
            <li key={item.id} className={`bg-background rounded-xl p-3 ring-1 flex items-start gap-3 ${item.done_at ? "ring-success/20" : "ring-border"}`}>
              <button onClick={() => toggleItem(item.id)} className={`size-5 rounded-md ring-1 shrink-0 flex items-center justify-center mt-0.5 ${item.done_at ? "bg-success ring-success text-white" : "ring-border hover:ring-success/40"}`}>
                {item.done_at && <CheckIcon className="size-3.5" />}
              </button>
              <input value={item.title} onChange={(e) => updateItem(item.id, { title: e.target.value })} className={`${inputClass} flex-1 ${item.done_at ? "text-subtle line-through" : ""}`} />
              <button onClick={() => removeItem(item.id)} className="p-1 text-subtle hover:text-danger"><TrashIcon className="size-3.5" /></button>
            </li>
          ))}
        </ul>
      ...
      {/* Notes */}
      <div className="bg-surface rounded-2xl p-5 border border-border">
        <h2 className="text-sm font-semibold text-foreground mb-3">Notes (markdown)</h2>
