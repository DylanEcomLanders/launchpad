  ...
  return (
    <div className="min-h-screen bg-surface">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-8">
          <h1 className="text-[28px] leading-tight font-bold text-foreground">Deck Builder</h1>
        ...
        <div className="space-y-6">
          ...
          <section className="border border-border rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={brandName} onChange={e => setBrandName(e.target.value)} placeholder="e.g. Ecomlanders" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
              <div>
                <label className="block text-xs text-subtle mb-1">URL</label>
                <input value={brandUrl} onChange={e => setBrandUrl(e.target.value)} placeholder="https://ecomlanders.app" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
            </div>
          </section>
          ...
          <section className="border border-border rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <input type="number" value={traffic} onChange={e => setTraffic(e.target.value)} placeholder="150000" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
              <div>
                <label className="block text-xs text-subtle mb-1">Current CVR (%)</label>
                <input type="number" step="0.1" value={cvr} onChange={e => setCvr(e.target.value)} placeholder="1.8" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
              <div>
                <label className="block text-xs text-subtle mb-1">Benchmark CVR (%)</label>
                <input type="number" step="0.1" value={benchmarkCvr} onChange={e => setBenchmarkCvr(e.target.value)} className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
              <div>
                <label className="block text-xs text-subtle mb-1">AOV (£)</label>
                <input type="number" value={aov} onChange={e => setAov(e.target.value)} placeholder="55" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
            </div>
          ...
          <section className="border border-border rounded-xl p-5">
            ...
            {priorities.map((p, i) => (
              <div key={i} className="flex items-center gap-3 mb-3">
                <span className="size-6 rounded-full bg-surface-raised flex items-center justify-center text-xs font-bold text-subtle shrink-0">{i + 1}</span>
                <input
                  value={p}
                  onChange={e => {
                  ...
                  className="flex-1 px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted"
                />
              </div>
            ))}
          </section>
          ...
          <section className="border border-border rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={retainerPrice} onChange={e => setRetainerPrice(e.target.value)} className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
              <div>
                <label className="block text-xs text-subtle mb-1">Anchor / Pro (£/mo)</label>
                <input type="number" value={anchorPrice} onChange={e => setAnchorPrice(e.target.value)} className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:border-muted" />
              </div>
            </div>
          </section>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="px-6 py-3 bg-white text-background text-sm font-semibold rounded-lg hover:bg-foreground disabled:opacity-40 transition-colors"
            >
              {generating ? "Generating..." : "Generate Deck"}
            </button>

            {deckUrl && (
              <div className="flex items-center gap-3 flex-1 bg-surface-raised border border-border rounded-lg px-4 py-2.5">
