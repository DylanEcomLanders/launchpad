  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      {!hydrated ? (
        ...
        <div className="bg-[#0F0F10] rounded-2xl p-12 text-center ring-1 ring-white/[0.04]"><p className="text-sm text-[#71757D]">No roadmaps yet. Start one per client.</p></div>
      ) : (
        <ul className="space-y-2">
          {roadmaps.map((r) => (
            <li key={r.id}>
              <Link href={`/tools/roadmap/${r.id}`} className="block bg-[#0F0F10] rounded-xl p-4 ring-1 ring-white/[0.04] hover:ring-cyan-500/30 transition-all">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-[#E5E5EA] truncate mb-1">
                    ...
                    <div className="text-[12px] text-[#71757D] flex items-center gap-2 flex-wrap">
                      ...
                      {HORIZONS.map((h) => {
                        ...
                        return count > 0 ? (
                          <span key={h} className="px-1.5 py-0.5 rounded bg-[#1A1A1A] text-[10px] uppercase tracking-wider">
                            D{h}: {count}
                          </span>
                        ) : null;
                      })}
