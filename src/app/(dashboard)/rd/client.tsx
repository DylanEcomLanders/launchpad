  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto px-6 md:px-10 py-16">
        <div className="text-[13px] text-[#7A7A7A]">Loading R&amp;D tracker...</div>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-6 md:px-10 py-10 pb-24">
      ...
      {(["parked", "shipped", "killed"] as const).map((status) => {
        ...
        return (
          <div key={status} className="mt-8">
            ...
            >
              <ChevronDownIcon
                className={`size-3.5 text-[#7A7A7A] transition-transform ${open ? "" : "-rotate-90"}`}
              />
              <span className="text-[11px] font-semibold uppercase tracking-wider text-[#7A7A7A] group-hover:text-[#1B1B1B] transition-colors">
                {status} ({items.length})
              </span>
            </button>
            {open && (
      ...
      <div className="mt-14">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h2 className="text-lg font-semibold text-[#1B1B1B] tracking-tight flex items-center gap-2">
              <LightBulbIcon className="size-4 text-[#1B1B1B]" />
              Ideas from the team
            </h2>
            <p className="text-[13px] text-[#7A7A7A] mt-1">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#E5E5EA] text-[#1B1B1B] text-[12px] font-medium rounded-lg hover:border-[#1B1B1B] transition-colors"
          >
            <PlusIcon className="size-3.5" />
            Add idea
          </button>
        </div>
        <div className="bg-[#F7F8FA] border border-[#EDEDEF] rounded-2xl p-3 md:p-4">
          {inboxIdeas.length === 0 ? (
            <div className="px-3 py-10 text-center text-[13px] text-[#7A7A7A]">
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0 flex-1">
          <div className="text-[15px] font-medium text-[#1B1B1B] truncate">
            {initiative.name}
          </div>
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        {stale ? (
          ...
          <span className="text-[11px] font-medium text-[#1B1B1B] tabular-nums">
            {pct}%
          </span>
        )}
      </div>
...
function IdeaCard({ idea }: { idea: Idea }) {
  return (
    <Link
      ...
      className="block bg-white border border-[#EDEDEF] rounded-lg p-3 hover:border-[#1B1B1B]/30 transition-colors"
    >
      <div className="flex items-start justify-between gap-2 mb-1">
        <div className="text-[14px] font-medium text-[#1B1B1B] flex-1 min-w-0">
          {idea.title}
        </div>
        <div className="text-[11px] text-[#A0A0A0] shrink-0">
...
function EmptyState({
  ...
  return (
    <div className="bg-[#F7F8FA] border border-dashed border-[#E5E5EA] rounded-2xl p-10 text-center">
      <div className="text-[15px] font-medium text-[#1B1B1B] mb-1">{title}</div>
      <div className="text-[13px] text-[#7A7A7A] mb-5">{body}</div>
      <button
        onClick={cta.onClick}
        className="inline-flex items-center px-4 py-2 bg-[#1B1B1B] text-white text-[13px] font-medium rounded-lg hover:bg-[#2D2D2D] transition-colors"
      >
        {cta.label}
      </button>
    </div>
