    ...
    <div className="mx-auto max-w-6xl px-6 py-8 md:px-10">
              ...
              return (
                <div key={r.pod.id} className="rounded-lg border border-border bg-surface p-3">
                  ...
                  <div className="mt-1 text-lg font-semibold tabular-nums text-foreground">
                    {r.available} <span className="text-[11px] font-normal text-subtle">pts open</span>
                  </div>
      ...
      <div className="mb-2.5 flex items-center justify-between">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-subtle">
          Leads <span className="text-subtle">({leads.length})</span>
        </h3>
          ...
          <tbody className="divide-y divide-border">
            {leads.map((l) => (
              <tr key={l.id} className="hover:bg-background">
                <td className="px-4 py-2.5 font-medium text-foreground">{l.company}</td>
                <td className="px-4 py-2.5 text-subtle">
                  <div>{l.contact}</div>
                  <div className="text-[11px] text-subtle">{l.email}</div>
                </td>
                <td className="px-4 py-2.5 text-[12px] text-subtle">{l.source}</td>
      ...
      {csvOpen && (
        <div className="fixed inset-0 z-40 grid place-items-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-xl border border-border bg-surface p-6 shadow-[var(--shadow-elevated)]">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-base font-semibold text-foreground">Import leads from CSV</h3>
                <p className="mt-0.5 text-[12px] text-subtle">
                  Paste rows as <code>company, contact, email, value</code>. First line is the header.
                </p>
              </div>
              <button onClick={() => setCsvOpen(false)} className="text-subtle hover:text-foreground">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button
                ...
                className="rounded-lg border border-border px-3 py-1.5 text-sm font-medium text-subtle hover:text-foreground"
              >
                Cancel
              </button>
              <button
