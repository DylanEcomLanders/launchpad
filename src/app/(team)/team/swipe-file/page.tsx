  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 py-12 md:py-20">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl md:text-3xl font-bold mb-1">Swipe File</h1>
        ...
        <form
          onSubmit={onAdd}
          className="bg-[#181818] border border-[#2A2A2A] rounded-lg p-4 mb-8 flex flex-wrap items-center gap-3"
        >
          <input
            type="url"
            value={urlInput}
            ...
            required
            disabled={submitting}
            className="flex-1 min-w-[260px] text-sm px-3 py-2 border border-[#2A2A2A] rounded-md focus:outline-none focus:border-[#999] disabled:bg-[#F7F7F8]"
          />
          <button
            type="submit"
            disabled={submitting || !urlInput.trim()}
            className="px-4 py-2 bg-[#222222] text-[#E5E5EA] text-xs font-medium rounded-md hover:bg-[#2A2A2A] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
          >
          ...
          <div className="flex items-center gap-2 mb-5 flex-wrap">
            <button
              ...
              className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${
                !tagFilter
                  ? "bg-[#222222] text-[#E5E5EA] border-[#2A2A2A]"
        ...
        {loading ? (
          ...
          <div className="bg-[#181818] border border-[#2A2A2A] rounded-lg py-14 text-center">
...
function SwipeCard({ entry, onClick }: { entry: SwipeEntry; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group bg-[#181818] border border-[#2A2A2A] rounded-lg overflow-hidden hover:border-[#1B1B1B] hover:shadow-sm transition-all text-left flex flex-col"
    >
      <div className="aspect-[16/10] bg-[#F7F7F8] overflow-hidden">
        {entry.desktopUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-[10px] text-[#9CA3AF] uppercase tracking-wider">No screenshot</div>
        )}
      </div>
      <div className="px-3.5 py-2.5">
        <p className="text-sm font-medium text-[#E5E5EA] truncate">{entry.title || hostnameFromUrl(entry.url)}</p>
        <p className="text-[10px] text-[#9CA3AF] truncate mt-0.5">{hostnameFromUrl(entry.url)}</p>
        {entry.tags.length > 0 && (
          <div className="flex items-center gap-1 mt-2 flex-wrap">
            {entry.tags.slice(0, 3).map((t) => (
              <span key={t} className="text-[9px] uppercase tracking-wider text-[#9CA3AF] bg-[#222222] px-1.5 py-0.5 rounded-full">
                {t}
              </span>
            ))}
            {entry.tags.length > 3 && <span className="text-[9px] text-[#9CA3AF]">+{entry.tags.length - 3}</span>}
          </div>
        )}
      </div>
    </button>
...
function SwipeDetail({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4 md:p-8">
      <div className="relative w-full max-w-6xl max-h-[92vh] bg-[#181818] rounded-xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-10 p-1.5 rounded-lg bg-[#181818]/90 text-[#9CA3AF] hover:text-[#E5E5EA] hover:bg-[#181818] shadow-sm"
          aria-label="Close"
        >
        ...
        <div className="flex-1 bg-[#F7F7F8] flex flex-col min-h-0">
          <div className="flex items-center justify-center gap-1 p-3 border-b border-[#2A2A2A] bg-[#181818]">
              ...
              {(["desktop", "mobile"] as const).map((v) => (
                <button
                  ...
                  className={`px-3 py-1 text-[10px] font-semibold uppercase tracking-wider rounded transition-colors ${
                    view === v ? "bg-[#181818] text-[#E5E5EA] shadow-sm" : "text-[#9CA3AF] hover:text-[#E5E5EA]"
                  }`}
                >
                  {v}
                </button>
          ...
          <div className="flex-1 overflow-auto flex items-start justify-center p-4">
            {activeImage ? (
              ...
              <p className="text-xs text-[#9CA3AF] mt-10">No {view} screenshot</p>
            )}
          </div>
        </div>

        {/* Sidebar */}
        <div className="w-full md:w-80 border-t md:border-t-0 md:border-l border-[#2A2A2A] flex flex-col">
          <div className="p-5 space-y-4 overflow-y-auto flex-1">
            <div>
              <input
                ...
                onBlur={saveTitle}
                placeholder="Title"
                className="w-full text-sm font-semibold text-[#E5E5EA] bg-transparent border-0 px-0 py-0 focus:outline-none focus:ring-0"
              />
              <a
                href={entry.url}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-[10px] text-[#9CA3AF] hover:text-[#E5E5EA] mt-1 truncate max-w-full"
              >
            ...
            <div>
              <label className="text-[9px] font-semibold uppercase tracking-wider text-[#9CA3AF]">Tags</label>
              <input
                type="text"
                value={tagsInput}
                ...
                className="mt-1 w-full text-xs px-2.5 py-1.5 border border-[#2A2A2A] rounded-md focus:outline-none focus:border-[#999]"
              />
              <p className="text-[10px] text-[#9CA3AF] mt-1">Comma-separated</p>
            </div>

            <div>
              <label className="text-[9px] font-semibold uppercase tracking-wider text-[#9CA3AF]">Notes</label>
              <textarea
                value={notes}
                ...
                className="mt-1 w-full text-xs px-2.5 py-2 border border-[#2A2A2A] rounded-md focus:outline-none focus:border-[#999] resize-none"
              />
            </div>
          </div>
