  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 py-12 md:py-20">
        ...
        <form
          onSubmit={onAdd}
          className="bg-surface border border-border rounded-lg p-4 mb-8 flex flex-wrap items-center gap-3"
        >
          <input
            type="url"
            value={urlInput}
          ...
          <button
            ...
            className="px-4 py-2 bg-surface-raised text-foreground text-xs font-medium rounded-md hover:bg-border disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
          >
    ...
    >
      <div className="aspect-[16/10] bg-surface-raised overflow-hidden">
        {entry.desktopUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-[10px] text-muted uppercase tracking-wider">No screenshot</div>
        )}
      </div>
      <div className="px-3.5 py-2.5">
        <p className="text-sm font-medium text-foreground truncate">{entry.title || hostnameFromUrl(entry.url)}</p>
        <p className="text-[10px] text-muted truncate mt-0.5">{hostnameFromUrl(entry.url)}</p>
        {entry.tags.length > 0 && (
          <div className="flex items-center gap-1 mt-2 flex-wrap">
            {entry.tags.slice(0, 3).map((t) => (
              <span key={t} className="text-[9px] uppercase tracking-wider text-muted bg-surface-raised px-1.5 py-0.5 rounded-full">
                {t}
              </span>
            ))}
            {entry.tags.length > 3 && <span className="text-[9px] text-muted">+{entry.tags.length - 3}</span>}
          </div>
        )}
      </div>
    </button>
...
function SwipeDetail({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4 md:p-8">
      <div className="relative w-full max-w-6xl max-h-[92vh] bg-surface rounded-xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-10 p-1.5 rounded-lg bg-surface/90 text-muted hover:text-foreground hover:bg-surface shadow-sm"
          aria-label="Close"
        >
        ...
        <div className="flex-1 bg-surface-raised flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-auto flex items-start justify-center p-4">
            {activeImage ? (
              ...
              <p className="text-xs text-muted mt-10">No {view} screenshot</p>
            )}
          </div>
        </div>
          ...
          <div className="p-5 space-y-4 overflow-y-auto flex-1">
            <div>
              <input
                ...
                onBlur={saveTitle}
                placeholder="Title"
                className="w-full text-sm font-semibold text-foreground bg-transparent border-0 px-0 py-0 focus:outline-none focus:ring-0"
              />
              <a
                href={entry.url}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-[10px] text-muted hover:text-foreground mt-1 truncate max-w-full"
              >
            ...
            <div>
              <label className="text-[9px] font-semibold uppercase tracking-wider text-muted">Tags</label>
              <input
                type="text"
                value={tagsInput}
                ...
                className="mt-1 w-full text-xs px-2.5 py-1.5 border border-border rounded-md focus:outline-none focus:border-subtle"
              />
              <p className="text-[10px] text-muted mt-1">Comma-separated</p>
            </div>

            <div>
              <label className="text-[9px] font-semibold uppercase tracking-wider text-muted">Notes</label>
              <textarea
                value={notes}
                ...
                className="mt-1 w-full text-xs px-2.5 py-2 border border-border rounded-md focus:outline-none focus:border-subtle resize-none"
              />
            </div>
          </div>
