...
export default function TeamInvoicePage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-12">
        <div className="max-w-md w-full text-center">
          ...
          <h1 className="text-2xl font-bold text-[#1B1B1B] mb-3 tracking-tight">
            Invoice received.
          </h1>
          ...
          <div className="flex gap-2 justify-center">
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-[#7A7A7A] hover:text-[#1B1B1B] transition-colors"
            >
              Submit another
            </button>
            <Link
              href="/team/payments"
              className="px-4 py-2 bg-[#1B1B1B] text-white text-sm font-medium rounded-lg hover:bg-[#2D2D2D] transition-colors"
            >
              Back to payments
            </Link>
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 md:px-10 py-10 pb-24">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-[#1B1B1B] tracking-tight mb-2">
          Submit an invoice
        </h1>
        <p className="text-sm text-[#7A7A7A] leading-relaxed max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Field label="Attach invoice (PDF or image)" required>
          {file ? (
            <div className="flex items-center justify-between gap-3 px-4 py-3 bg-white border border-[#E5E5EA] rounded-lg shadow-[var(--shadow-soft)]">
              <div className="flex items-center gap-2 min-w-0">
                <DocumentArrowUpIcon className="size-4 text-[#7A7A7A] shrink-0" />
              ...
              <button
                type="button"
                onClick={removeFile}
                className="text-[#7A7A7A] hover:text-[#B22B2B] transition-colors p-1"
            ...
            <label className="flex flex-col items-center justify-center gap-2 py-10 border-2 border-dashed border-[#E5E5EA] rounded-lg cursor-pointer hover:border-[#999] transition-colors bg-white">
              {uploading ? (
                <div className="flex items-center gap-2 text-[#7A7A7A] text-sm">
                  <div className="size-4 border-2 border-[#E5E5EA] border-t-[#1B1B1B] rounded-full animate-spin" />
                  Uploading...
                </div>
              ) : (
                <>
                  <DocumentArrowUpIcon className="size-6 text-[#A0A0A0]" />
        ...
        {/* Error */}
        {error && (
          <div className="px-4 py-3 bg-[#FDECEA] border border-[#F5BFBA] rounded-lg text-sm text-[#B22B2B]">
            {error}
          </div>
        )}
        ...
        <button
          ...
          className="w-full py-3.5 bg-[#1B1B1B] text-white text-sm font-semibold rounded-lg hover:bg-[#2D2D2D] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
