...
export default function WhyPodsPage() {
  return (
    <div className="mx-auto max-w-5xl px-6 py-10 md:px-10">
      ...
      {/* Hero */}
      <div className="mt-6 rounded-2xl border border-border bg-surface p-8 shadow-[var(--shadow-soft)] md:p-12">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-success">
          Internal · For the team · Reference v1
        </p>
        <h1 className="mt-3 text-4xl font-semibold leading-[1.05] text-foreground md:text-5xl">
          How we work,<br />
          and <span className="italic text-success">why</span>.
        </h1>
        <p className="mt-5 max-w-2xl text-sm leading-relaxed text-subtle md:text-base">
      ...
      {/* Why we changed */}
      <Section number="01" eyebrow="Why we changed" title={<>The old way didn't work.<br /><span className="italic text-success">We were burning out.</span></>}>
        ...
        <p className="mt-3 text-sm leading-relaxed text-muted md:text-[15px]">
          The system below isn't extra rules — it's protection. <strong className="text-foreground">Protection for your hours, your focus, and the work being good.</strong>
        </p>
      </Section>

      {/* Headline */}
      <Section number="02" eyebrow="In one sentence" title={<>Same work. Predictable rhythm.<br /><span className="italic text-success">One day, one job.</span></>}>
        <p className="text-sm leading-relaxed text-muted md:text-[15px]">
      ...
      <Section number="03" eyebrow="Cadence" title="Each day has one job. Thursday is the only day clients hear from us.">
        <p className="mb-5 text-sm leading-relaxed text-muted md:text-[15px]">
        ...
        <div className="grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-border bg-foreground md:grid-cols-5">
          {WEEK.map((d) => (
            <div
              key={d.day}
              className={`flex flex-col gap-2.5 p-5 ${d.client ? "bg-success" : "bg-surface"}`}
            >
              <div className={`text-2xl font-semibold leading-none ${d.client ? "text-success" : "text-foreground"}`}>
                {d.day}
              </div>
              <div className="text-[10px] font-semibold uppercase tracking-wider text-subtle">
                {d.tag}
              </div>
              <div className={`text-[10px] font-semibold uppercase tracking-wider ${d.client ? "text-success" : "text-foreground"}`}>
                {d.job}
              </div>
              <ul className="mt-1 space-y-1.5 text-[12px] leading-snug text-muted">
                {d.bullets.map((b) => (
      ...
      <Section number="04" eyebrow="Your pod, your team" title="Three pods of four. One senior and one junior on each side.">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          {POD_DEFINITIONS.map((pod) => (
            <div key={pod.name} className="rounded-xl border border-border bg-surface p-5 shadow-[var(--shadow-soft)]">
              <div className="text-2xl font-semibold italic text-success">{pod.name}</div>
              <div className="mt-1 text-[10px] font-semibold uppercase tracking-wider text-subtle">
                {pod.tagline}
              </div>
              <div className="mt-4 divide-y divide-border">
                {pod.members.map((m) => (
                  ...
                  >
                    <span
                      className={`text-sm font-medium ${m.placeholder ? "italic text-success" : "text-foreground"}`}
                    >
                      {m.name}
                    </span>
      ...
      <Section number="05" eyebrow="How project size works" title="Three buckets. Three durations. Set on Monday, delivered on Thursday.">
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-surface shadow-[var(--shadow-soft)]">
          {BUCKETS.map((b, i) => (
            <div
              key={b.letter}
              className={`grid grid-cols-[60px_120px_1fr_auto] items-center gap-6 px-6 py-5 ${i > 0 ? "border-t border-border" : ""}`}
            >
              <div className="text-5xl font-semibold italic leading-none text-success">
                {b.letter}
              </div>
              <div>
                <div className="text-2xl font-medium text-foreground">{b.points}</div>
                <div className="text-[10px] font-semibold uppercase tracking-wider text-subtle">
                  points
                </div>
              </div>
              <div className="text-sm leading-relaxed text-muted">{b.description}</div>
              <div className="text-xs font-semibold tracking-wider text-success">
                {b.duration}
              </div>
            </div>
          ))}
      ...
      <Section number="06" eyebrow="The Monday Protocol" title="Standard kicks off Monday. Rush is the only mid-week exception.">
        ...
        <div className="grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-border bg-foreground md:grid-cols-2">
          <div className="bg-surface p-6">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-subtle">
              Standard — the default
            </div>
            <div className="mt-2 text-3xl font-semibold leading-tight text-foreground">
              Monday <span className="text-base font-normal text-subtle">kickoff</span>
            </div>
            <ul className="mt-4 space-y-2 text-[13px] leading-snug text-muted">
              {[
                "Sign by Friday 5pm",
          ...
          <div className="bg-[#FEF3C7] p-6">
            ...
            <div className="mt-2 text-3xl font-semibold leading-tight text-warning">
      ...
      <Section number="07" eyebrow="What you get back" title={<>Predictable Thursdays. Protected hours.<br /><span className="italic text-success">Work that's actually yours.</span></>}>
        <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-3">
          {PRACTICE.map((p) => (
            <div key={p.num} className="rounded-xl border border-border bg-surface p-6 shadow-[var(--shadow-soft)]">
              <div className="text-4xl font-semibold italic leading-none text-success">
                {p.num}
              </div>
      ...
      {/* Sign-off */}
      <div className="mt-10 rounded-2xl border border-border bg-background p-8 text-center md:p-12">
        <p className="text-sm italic leading-relaxed text-muted md:text-base">
          Questions, ask your pod's senior, or Alister. The full operating system docs are in the team folder. <strong className="not-italic text-foreground">Welcome to the new rhythm.</strong>
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <Link
            href="/pods-v2?view=team"
            className="rounded-lg bg-surface-raised px-4 py-2 text-xs font-medium text-foreground hover:bg-border"
          >
            Open your pod board →
          </Link>
          <Link
            href="/team"
            className="rounded-lg border border-border bg-surface px-4 py-2 text-xs font-medium text-foreground hover:border-surface"
          >
            Back to Team Tools
          </Link>
        </div>
...
function Section({
  ...
  return (
    <section className="mt-12">
      <div className="mb-4 flex items-baseline gap-3 text-[11px] font-semibold uppercase tracking-wider text-subtle">
        <span className="text-foreground">{number}</span>
        <span>·</span>
        <span className="text-success">{eyebrow}</span>
      </div>
      <h2 className="text-2xl font-semibold leading-tight text-foreground md:text-[32px] md:leading-[1.15]">
        {title}
      </h2>
