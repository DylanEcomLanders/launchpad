...
export default function WhyPodsPage() {
  return (
    <div className="mx-auto max-w-5xl px-6 py-10 md:px-10">
      ...
      {/* Hero */}
      <div className="mt-6 rounded-2xl border border-[#E5E5EA] bg-white p-8 shadow-[var(--shadow-soft)] md:p-12">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-[#16A34A]">
          Internal · For the team · Reference v1
        </p>
        <h1 className="mt-3 text-4xl font-semibold leading-[1.05] tracking-tight text-[#1B1B1B] md:text-5xl">
          How we work,<br />
          and <span className="italic text-[#16A34A]">why</span>.
        </h1>
        <p className="mt-5 max-w-2xl text-sm leading-relaxed text-[#7A7A7A] md:text-base">
      ...
      <Section number="01" eyebrow="Why we changed" title={<>The old way didn't work.<br /><span className="italic text-[#16A34A]">We were burning out.</span></>}>
        <p className="text-sm leading-relaxed text-[#555] md:text-[15px]">
        ...
        <p className="mt-3 text-sm leading-relaxed text-[#555] md:text-[15px]">
          The system below isn't extra rules — it's protection. <strong className="text-[#1B1B1B]">Protection for your hours, your focus, and the work being good.</strong>
        </p>
      </Section>
      ...
      <Section number="03" eyebrow="Cadence" title="Each day has one job. Thursday is the only day clients hear from us.">
        <p className="mb-5 text-sm leading-relaxed text-[#555] md:text-[15px]">
        ...
        <div className="grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-[#E5E5EA] bg-[#E5E5EA] md:grid-cols-5">
          {WEEK.map((d) => (
            <div
              key={d.day}
              className={`flex flex-col gap-2.5 p-5 ${d.client ? "bg-[#F0FDF4]" : "bg-white"}`}
            >
              <div className={`text-2xl font-semibold leading-none ${d.client ? "text-[#16A34A]" : "text-[#1B1B1B]"}`}>
                {d.day}
              </div>
              <div className="text-[10px] font-semibold uppercase tracking-wider text-[#A0A0A0]">
                {d.tag}
              </div>
              <div className={`text-[10px] font-semibold uppercase tracking-wider ${d.client ? "text-[#16A34A]" : "text-[#1B1B1B]"}`}>
                {d.job}
              </div>
              <ul className="mt-1 space-y-1.5 text-[12px] leading-snug text-[#555]">
                {d.bullets.map((b) => (
      ...
      <Section number="04" eyebrow="Your pod, your team" title="Three pods of four. One senior and one junior on each side.">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          {POD_DEFINITIONS.map((pod) => (
            <div key={pod.name} className="rounded-xl border border-[#E5E5EA] bg-white p-5 shadow-[var(--shadow-soft)]">
              <div className="text-2xl font-semibold italic text-[#16A34A]">{pod.name}</div>
              <div className="mt-1 text-[10px] font-semibold uppercase tracking-wider text-[#A0A0A0]">
                {pod.tagline}
              </div>
              <div className="mt-4 divide-y divide-[#E5E5EA]">
                {pod.members.map((m) => (
                  ...
                  >
                    <span
                      className={`text-sm font-medium ${m.placeholder ? "italic text-[#16A34A]" : "text-[#1B1B1B]"}`}
                    >
                      {m.name}
                    </span>
      ...
      <Section number="05" eyebrow="How project size works" title="Three buckets. Three durations. Set on Monday, delivered on Thursday.">
        ...
        <div className="overflow-hidden rounded-xl border border-[#E5E5EA] bg-white shadow-[var(--shadow-soft)]">
          {BUCKETS.map((b, i) => (
            <div
              key={b.letter}
              className={`grid grid-cols-[60px_120px_1fr_auto] items-center gap-6 px-6 py-5 ${i > 0 ? "border-t border-[#E5E5EA]" : ""}`}
            >
              <div className="text-5xl font-semibold italic leading-none text-[#16A34A]">
                {b.letter}
              </div>
              <div>
                <div className="text-2xl font-medium text-[#1B1B1B]">{b.points}</div>
                <div className="text-[10px] font-semibold uppercase tracking-wider text-[#A0A0A0]">
                  points
                </div>
              </div>
              <div className="text-sm leading-relaxed text-[#555]">{b.description}</div>
      ...
      <Section number="06" eyebrow="The Monday Protocol" title="Standard kicks off Monday. Rush is the only mid-week exception.">
        ...
        <div className="grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-[#E5E5EA] bg-[#E5E5EA] md:grid-cols-2">
          <div className="bg-white p-6">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-[#A0A0A0]">
              Standard — the default
            </div>
            <div className="mt-2 text-3xl font-semibold leading-tight text-[#1B1B1B]">
              Monday <span className="text-base font-normal text-[#A0A0A0]">kickoff</span>
            </div>
            <ul className="mt-4 space-y-2 text-[13px] leading-snug text-[#555]">
              {[
                "Sign by Friday 5pm",
      ...
      <Section number="07" eyebrow="What you get back" title={<>Predictable Thursdays. Protected hours.<br /><span className="italic text-[#16A34A]">Work that's actually yours.</span></>}>
        <div className="mt-2 grid grid-cols-1 gap-3 md:grid-cols-3">
          {PRACTICE.map((p) => (
            <div key={p.num} className="rounded-xl border border-[#E5E5EA] bg-white p-6 shadow-[var(--shadow-soft)]">
              <div className="text-4xl font-semibold italic leading-none text-[#16A34A]">
                {p.num}
              </div>
              <h3 className="mt-3 text-base font-semibold leading-snug text-[#1B1B1B]">
                {p.title}
              </h3>
              <p className="mt-2 text-[13px] leading-relaxed text-[#555]">{p.body}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      {/* Sign-off */}
      <div className="mt-10 rounded-2xl border border-[#E5E5EA] bg-[#F7F8FA] p-8 text-center md:p-12">
        <p className="text-sm italic leading-relaxed text-[#555] md:text-base">
          Questions, ask your pod's senior, or Alister. The full operating system docs are in the team folder. <strong className="not-italic text-[#1B1B1B]">Welcome to the new rhythm.</strong>
        </p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <Link
            href="/pods-v2?view=team"
            className="rounded-lg bg-[#1B1B1B] px-4 py-2 text-xs font-medium text-white hover:bg-[#2D2D2D]"
          >
            Open your pod board →
          </Link>
          <Link
            href="/team"
            className="rounded-lg border border-[#E5E5EA] bg-white px-4 py-2 text-xs font-medium text-[#1B1B1B] hover:border-[#1B1B1B]"
          >
            Back to Team Tools
          </Link>
        </div>
...
function Section({
  ...
  return (
    <section className="mt-12">
      <div className="mb-4 flex items-baseline gap-3 text-[11px] font-semibold uppercase tracking-wider text-[#A0A0A0]">
        <span className="text-[#1B1B1B]">{number}</span>
        <span>·</span>
        <span className="text-[#16A34A]">{eyebrow}</span>
      </div>
      <h2 className="text-2xl font-semibold leading-tight tracking-tight text-[#1B1B1B] md:text-[32px] md:leading-[1.15]">
        {title}
      </h2>
