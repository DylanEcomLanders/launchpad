...
function E({ k, content, onChange, className, tag, editable }: { k: ContentKey; content: Record<string, string>; onChange: (k: ContentKey, v: string) => void; className?: string; tag?: "p" | "h1" | "h2" | "span"; editable?: boolean }) {
  ...
  return (
    <Tag
      ...
      className={`outline-none ${editable ? "focus:ring-2 focus:ring-success/30 focus:rounded-sm cursor-text" : ""} ${className || ""}`}
...
function ReferralProgrammeInner() {
  ...
  return (
    <div className="min-h-screen relative">
      ...
      {/* ── Hero (Dark) ── */}
      <section className="bg-background text-white px-6 py-20 md:py-28">
        <div className="max-w-2xl mx-auto text-center">
          <div className="mb-10">
            <Logo height={18} />
          </div>
          <E k="heroLabel" content={content} onChange={update} editable={canEdit} tag="p" className="text-xs font-semibold uppercase tracking-[0.2em] text-success mb-4" />
          ...
          <E k="heroSubtitle" content={content} onChange={update} editable={canEdit} tag="p" className="text-base md:text-lg text-subtle leading-relaxed max-w-lg mx-auto" />
        </div>
      </section>
      ...
      <section className="bg-white px-6 py-16 md:py-20">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-0 border border-foreground rounded-2xl overflow-hidden">
            <div className="p-10 text-center border-r border-foreground">
              <E k="comm1Value" content={content} onChange={update} editable={canEdit} tag="p" className="text-5xl md:text-6xl font-bold text-surface tracking-tight" />
              <E k="comm1Desc" content={content} onChange={update} editable={canEdit} tag="p" className="text-sm text-subtle mt-3" />
            </div>
            <div className="p-10 text-center bg-[#F8FFF8]">
              <E k="comm2Value" content={content} onChange={update} editable={canEdit} tag="p" className="text-5xl md:text-6xl font-bold text-success tracking-tight" />
              <E k="comm2Desc" content={content} onChange={update} editable={canEdit} tag="p" className="text-sm text-subtle mt-3" />
            </div>
          </div>
          <E k="commFootnote" content={content} onChange={update} editable={canEdit} tag="p" className="text-[11px] text-muted text-center mt-4" />
        </div>
      </section>

      {/* ── How It Works (Dark) ── */}
      <section className="bg-background text-white px-6 py-16 md:py-20">
        <div className="max-w-2xl mx-auto">
          <E k="howLabel" content={content} onChange={update} editable={canEdit} tag="p" className="text-xs font-semibold uppercase tracking-[0.2em] text-success mb-10 text-center" />
          <div className="space-y-12">
            <div className="flex items-start gap-6">
              <div className="shrink-0 size-12 rounded-full border-2 border-border flex items-center justify-center text-lg font-bold text-white">1</div>
              <div>
                <E k="step1Title" content={content} onChange={update} editable={canEdit} tag="p" className="text-lg font-bold mb-1" />
                <E k="step1Desc" content={content} onChange={update} editable={canEdit} tag="p" className="text-sm text-subtle leading-relaxed" />
              </div>
            </div>
            <div className="flex items-start gap-6">
              <div className="shrink-0 size-12 rounded-full border-2 border-border flex items-center justify-center text-lg font-bold text-white">2</div>
              <div>
                <E k="step2Title" content={content} onChange={update} editable={canEdit} tag="p" className="text-lg font-bold mb-1" />
                <E k="step2Desc" content={content} onChange={update} editable={canEdit} tag="p" className="text-sm text-subtle leading-relaxed" />
              </div>
            </div>
            <div className="flex items-start gap-6">
              <div className="shrink-0 size-12 rounded-full border-2 border-success flex items-center justify-center text-lg font-bold text-success">3</div>
              <div>
                <E k="step3Title" content={content} onChange={update} editable={canEdit} tag="p" className="text-lg font-bold mb-1" />
                <E k="step3Desc" content={content} onChange={update} editable={canEdit} tag="p" className="text-sm text-subtle leading-relaxed" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-white px-6 py-16 md:py-20">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-foreground rounded-2xl p-8">
              <E k="ex1Label" content={content} onChange={update} editable={canEdit} tag="p" className="text-xs text-muted uppercase tracking-wider mb-2" />
              <E k="ex1Value" content={content} onChange={update} editable={canEdit} tag="p" className="text-3xl font-bold text-surface" />
              <E k="ex1Desc" content={content} onChange={update} editable={canEdit} tag="p" className="text-xs text-subtle mt-1" />
            </div>
            <div className="border border-foreground rounded-2xl p-8 bg-[#F8FFF8]">
              <E k="ex2Label" content={content} onChange={update} editable={canEdit} tag="p" className="text-xs text-muted uppercase tracking-wider mb-2" />
              <E k="ex2Value" content={content} onChange={update} editable={canEdit} tag="p" className="text-3xl font-bold text-success" />
              <E k="ex2Desc" content={content} onChange={update} editable={canEdit} tag="p" className="text-xs text-subtle mt-1" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-background text-white px-6 py-16 md:py-20">
        <div className="max-w-2xl mx-auto">
          <E k="buildLabel" content={content} onChange={update} editable={canEdit} tag="p" className="text-xs font-semibold uppercase tracking-[0.2em] text-success mb-8 text-center" />
      ...
      <section className="bg-white px-6 py-20 md:py-28">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-4">
            <a href={content.whatsappUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 px-8 py-4 bg-background text-white text-sm font-semibold rounded-xl hover:bg-surface transition-colors">
            ...
            <a href={content.calUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-4 border-2 border-foreground text-surface text-sm font-semibold rounded-xl hover:border-surface transition-colors">
              Book a Call
            </a>
          </div>
        </div>
      ...
      <div className="bg-background px-6 py-8 text-center">
        <Logo height={14} />
        <p className="text-[10px] text-subtle mt-3">© 2026 Ecomlanders. All rights reserved.</p>
      </div>
    </div>
  );
}
